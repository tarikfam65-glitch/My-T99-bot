# ===================== bot.py (النسخة النهائية - مع التعديلات الأمنية والهيكلية) =====================
# -*- coding: utf-8 -*-
...
if not TELEGRAM_TOKEN:
    raise ValueError("❌ TELEGRAM_TOKEN غير مضبوط في متغيرات البيئة!")

ADMIN_ID = int(os.environ.get('ADMIN_ID', 0))
if ADMIN_ID == 0:
    raise ValueError("❌ ADMIN_ID غير مضبوط في متغيرات البيئة!")
...
def check_breach(email):
    """التحقق من تسريب البريد الإلكتروني عبر HaveIBeenPwned API"""
    try:
...
# ===================== دوال التواصل مع خادم الهاتف (مع دعم ngrok) =====================
...
def handle_callback(call):
    try:
        ...
        if data == "mode_site":
            ...
            text = "🛡️ <b>فحص تسريب البريد الإلكتروني</b>\n\nأرسل البريد الإلكتروني للتحقق من وجوده في قواعد البيانات المسربة (HaveIBeenPwned)."
            ...
            if chat_id != ADMIN_ID:
                ...
                if get_user_points(chat_id) < 30:
                    ...
                    text += "⚠️ <b>ملاحظة هامة:</b> للتحكم بالهاتف من خارج الشبكة المحلية، يجب استخدام خدمة ngrok على هاتف المستخدم:\n"
                    text += "1️⃣ ثبّت ngrok على الهاتف: `pkg install ngrok`\n"
                    text += "2️⃣ شغّل: `ngrok http 5000`\n"
                    text += "3️⃣ انسخ الرابط العام (مثل `https://abcd.ngrok.io`) واستخدمه بدلاً من الـ IP المحلي.\n\n"
                    text += "📝 <b>الأوامر المتاحة بعد اختيار الجهاز:</b>\n"
                    text += "موقع، كاميرا، لقطة، صور، شاشة، جهات اتصال، رسائل\n\n"
                    text += "🔹 للأجهزة المرتبطة كـ 'طفل'، تتوفر أوامر إضافية:\n"
                    text += "سجل، حظر <دومين>، الغاء حظر <دومين>، تطبيقات، تقارير"
...
def handle_text_messages(message):
    ...
    try:
        ...
        # ===== ميزة فحص التسريبات (OSINT) =====
        if state == "waiting_for_fb_hacked":
            if not re.match(r'^[^@\s]+@[^@\s]+\.[^@\s]+$', text):
                bot.send_message(chat_id, "❌ البريد الإلكتروني غير صحيح. أرسل بريداً صالحاً (مثل: user@example.com).")
                return
            bot.send_message(chat_id, "⏳ جاري فحص البريد الإلكتروني في قواعد البيانات المسربة...")
            result = check_breach(text)
            bot.send_message(chat_id, result, reply_markup=build_main_menu(chat_id))
            feature_usage["فحص التسريبات"] += 1
            user_states[chat_id] = None
            return
        ...
        # لا نفحص الروابط إلا إذا كان المستخدم مربوطاً وحالته None
        if chat_id in linked_users and state is None:
            device_info = secure_connected_devices.get(chat_id)
            # التحقق مما إذا كان الجهاز من نوع "طفل"
            if device_info and "طفل" in device_info.get('type', ''):
                ...
                for domain in urls:
                    if domain in blocked:
                        ...
                        return  # منع إرسال الرسالة الأصلية
            # فحص الروابط المشبوهة (لجميع المستخدمين المربوطين)
            urls = re.findall(r'https?://[^\s]+', text)
            for url in urls:
...
def handle_documents(message):
    ...
    try:
        ...
        # ✅ حذف الملف المؤقت دائماً (حتى في حالة الخطأ)
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                os.remove(temp_file_path)
                logger.info(f"تم حذف الملف المؤقت: {temp_file_path}")
            except Exception as del_err:
                logger.error(f"فشل حذف الملف المؤقت {temp_file_path}: {del_err}")
