...
# ===================== إعدادات التسجيل (مع إخفاء البيانات الحساسة) =====================
...
# ===================== إنشاء خادم Flask =====================
flask_app = Flask(__name__)  # ✅ تم إضافة تعريف flask_app

# ===================== المتغيرات العامة =====================
linked_users = set()          # ✅ تم إضافة المتغير المفقود
monitoring_data = {}          # ✅ تم إضافة المتغير المفقود
...
def init_db():
    with get_db_connection() as conn:
        ...
        # جدول الأجهزة المسجلة عبر Flask (Outbound)
        ...
        # جدول الأوامر المعلقة (Pending Commands)
        ...
        # جدول النقاط (يمكن الاستغناء عنه واستخدام حقل points في users)
        conn.commit()
...
def save_device(device_id, chat_id, device_type, real_id, endpoint, blocked_domains='[]', history='[]'):
    """حفظ جهاز مربوط في قاعدة البيانات"""
    with db_transaction() as conn:
        c = conn.cursor()
...
def get_device(device_id):
    """استرجاع بيانات جهاز من قاعدة البيانات"""
    with db_transaction() as conn:
        c = conn.cursor()
...
def get_blocked_domains(chat_id):
    """استرجاع قائمة الدومينات المحظورة لجهاز معين"""
...
def log_child_activity(chat_id, activity):
    """تسجيل نشاط الطفل (محاولة زيارة موقع محظور)"""
    devices = get_devices_by_chat(chat_id)
    if not devices:
        return
    for dev in devices:
...
def handle_callback(call):
    try:
        ...
        if data == "mode_site":
            ...
            # إرسال تعليمات Termux (سيتم إضافتها لاحقاً)
...
def handle_text_messages(message):
    ...
    try:
        ...
        if chat_id in linked_users and state is None:
            device_info = get_device(chat_id)  # يجب تعديل هذه الدالة
