...
"""
ShadowNet Framework v14.0 - الكود المصحح (الأخطاء فقط)
تم إصلاح: 409, googletrans, split, threading, دوال مفقودة, تصيد, كوكيز, كاميرا
"""
...
try:
    ...
    import feedparser
    from deep_translator import GoogleTranslator  # ✅ استبدال googletrans
    try:
...
def start_bot():
    while True:
        try:
            ...
            if "409" in str(e):
                logger.warning("⚠️ Conflict detected (409). Waiting 15 seconds before retry...")
                time.sleep(15)
            else:
...
if __name__ == '__main__':
    ...
    try:
        bot.delete_webhook()
        logger.info("✅ Webhook deleted at startup")
    except Exception as e:
        logger.warning(f"⚠️ Webhook deletion failed at startup: {e}")
