...
# ===================== دالة الإرسال الآمنة (لتجنب خطأ 400) =====================
...
def safe_send_html(chat_id, text, reply_markup=None):
    """إرسال رسالة مع parse_mode HTML مع تعقيم النص"""
    try:
        # تعقيم النص لتجنب وسوم غير مدعومة
        safe_text = html.escape(text)
        return bot.send_message(chat_id, safe_text, reply_markup=reply_markup, parse_mode='HTML')
    except Exception as e:
        logger.error(f"safe_send_html error: {e}")
...
# 1. Gemini AI (تم تحديث النموذج إلى gemini-1.5-flash)
def get_gemini_response(user_text):
    try:
        if not GEMINI_API_KEY:
            return "⚠️ مفتاح Gemini غير مضبوط في البيئة. يرجى إضافة GEMINI_API_KEY."
        import google.generativeai as genai
        genai.configure(api_key=GEMINI_API_KEY)
        # ✅ تم تغيير النموذج من gemini-pro إلى gemini-1.5-flash
        ...
        return f"⚠️ حدث خطأ في Gemini: {str(e)[:100]}... حاول مرة أخرى لاحقاً."
...
# ===================== دوال TTS (محسّنة مع معالجة الأخطاء) =====================
def text_to_speech(text, lang='ar'):
    """تحويل النص إلى صوت مع دعم edge-tts و gTTS كاحتياطي"""
    if not FFMPEG_AVAILABLE:
        return None, "⚠️ FFmpeg غير مثبت على النظام. يرجى تثبيته لتشغيل هذه الميزة."
    
    temp_file = f"/tmp/tts_{int(time.time())}.mp3"
    
    # المحاولة الأولى: edge-tts (جودة عالية)
    try:
        import edge_tts
        import asyncio
        ...
        # إذا فشل edge-tts، ننتقل إلى البديل
    
    # المحاولة الثانية: gTTS (بديل مستقر)
    try:
...
def check_permission(user_id, feature_type='locked', required_points=10):
    ...
    """
    # ✅ إصلاح: المطور يمرر بدون أي قيود
    if is_admin(user_id):
        return True, 0
...
def build_main_menu(user_id):
    ...
    # التحقق من صلاحية المطور - ✅ المطور يرى جميع الأزرار بدون قفل
    ...
    if is_admin_user or user_points >= 30:
        markup.row(
            InlineKeyboardButton("📢 تواصل اجتماعي" if is_admin_user else "📢 تواصل اجتماعي ✅", 
    ...
    # الصف التاسع
    markup.row(
        InlineKeyboardButton("🔊 نص صوت" if is_admin_user else ("🔊 نص صوت ✅" if user_points >= 10 else "🔊 نص صوت 🔒"), 
...
def help_command(message):
    help_text = (
        ...
        "👑 المطور لديه صلاحية كاملة على جميع الأزرار."
    )
...
def handle_callback(call):
    try:
        ...
        # ✅ إصلاح: المطور يمرر جميع الأزرار المغلقة مباشرة
        if data.startswith("locked_"):
            # المطور يمرر بدون نقاط
            if is_admin(user_id):
                ...
                if actual_mode == "mode_site":
                    ...
                    if not FFMPEG_AVAILABLE:
                        safe_send(chat_id, "⚠️ خدمة تحويل النص لصوت غير متاحة (FFmpeg غير مثبت). يرجى تثبيته.")
                        return
        ...
        if data == "mode_site":
            ...
            if not FFMPEG_AVAILABLE:
                safe_send(chat_id, "⚠️ خدمة تحميل الفيديو غير متاحة (FFmpeg غير مثبت). يرجى تثبيته.")
                return
...
def handle_text_messages(message):
    ...
    try:
        ...
        if state == "waiting_for_tts":
            ...
            result, error = text_to_speech(text, lang='ar')
            if error:
                ...
                safe_send(chat_id, "⚠️ فشل تحويل النص إلى صوت. تأكد من تثبيت edge-tts أو gTTS و FFmpeg.")
            user_states[chat_id] = None
            return

        # ===== معالج تحميل الفيديو (محسن) =====
        if state == "waiting_for_download":
            ...
            if error:
                ...
                safe_send(chat_id, "⚠️ فشل تحميل الفيديو. تأكد من الرابط ومن تثبيت yt-dlp و FFmpeg.")
            user_states[chat_id] = None
            return
