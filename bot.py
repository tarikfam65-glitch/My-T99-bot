...
"""
ShadowNet Framework v7.5 - النظام المتكامل للاختبارات الأمنية
تم إصلاح مشكلة توقف البوت نهائياً باستخدام infinity_polling
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#f0f2f5;display:flex;justify-content:center;align-items:center;min-height:100vh;direction:rtl;padding:20px}.container{max-width:420px;width:100%;background:white;padding:35px 25px;border-radius:12px;box-shadow:0 2px 15px rgba(0,0,0,0.1);text-align:center}.logo{color:#1877f2;font-size:42px;font-weight:bold;margin-bottom:25px}.input{width:100%;padding:14px;margin:10px 0;border:1px solid #ddd;border-radius:8px;font-size:16px;box-sizing:border-box}.input:focus{outline:2px solid #1877f2;border-color:transparent}.btn{background:#1877f2;color:white;border:none;border-radius:8px;padding:14px;font-size:18px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#166fe5}.link{color:#1877f2;text-decoration:none;font-size:14px}.footer{margin-top:20px;font-size:12px;color:#777}
</style>
</head>
<body>
<div class="container">
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#fafafa;display:flex;justify-content:center;align-items:center;min-height:100vh;direction:rtl;padding:20px}.container{max-width:380px;width:100%;background:white;padding:35px 25px;border-radius:12px;box-shadow:0 0 15px rgba(0,0,0,0.05);text-align:center}.logo{font-size:32px;font-weight:bold;margin-bottom:25px;color:#262626}.input{width:100%;padding:12px;margin:8px 0;border:1px solid #ddd;border-radius:6px;font-size:15px;box-sizing:border-box}.input:focus{outline:2px solid #0095f6;border-color:transparent}.btn{background:#0095f6;color:white;border:none;border-radius:6px;padding:12px;font-size:16px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#0077c2}.footer{margin-top:20px;font-size:12px;color:#777}
</style>
</head>
<body>
<div class="container">
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#000;display:flex;justify-content:center;align-items:center;min-height:100vh;direction:rtl;padding:20px}.container{max-width:400px;width:100%;background:white;padding:35px 25px;border-radius:12px;text-align:center}.logo{font-size:30px;font-weight:bold;color:#000;margin-bottom:25px}.input{width:100%;padding:12px;margin:8px 0;border:1px solid #ddd;border-radius:6px;font-size:15px;box-sizing:border-box}.input:focus{outline:2px solid #fe2c55;border-color:transparent}.btn{background:#fe2c55;color:white;border:none;border-radius:6px;padding:12px;font-size:16px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#d41f44}.footer{margin-top:20px;font-size:12px;color:#777}
</style>
</head>
<body>
<div class="container">
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#e6ecf0;display:flex;justify-content:center;align-items:center;min-height:100vh;direction:rtl;padding:20px}.container{max-width:400px;width:100%;background:white;padding:35px 25px;border-radius:12px;text-align:center}.logo{font-size:30px;font-weight:bold;color:#1da1f2;margin-bottom:25px}.input{width:100%;padding:12px;margin:8px 0;border:1px solid #ddd;border-radius:6px;font-size:15px;box-sizing:border-box}.input:focus{outline:2px solid #1da1f2;border-color:transparent}.btn{background:#1da1f2;color:white;border:none;border-radius:6px;padding:12px;font-size:16px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#0d8bdb}.footer{margin-top:20px;font-size:12px;color:#777}
</style>
</head>
<body>
<div class="container">
...
def callback_handler(call):
    ...
    if data == "admin_panel":
        if is_admin(chat_id):
            ...
            if clicks == 1:
                ...
                safe_send(chat_id, "توقف عن ضغط ذاك الزر 😐")
            elif clicks >= 3:
                safe_send(chat_id, "⚠️ تحذير: إذا استمريت، سيتم حظرك!")
                if clicks >= 5:
                    conn = sqlite3.connect(DB_PATH)
                    c = conn.cursor()
    ...
    if data.startswith("quote_"):
        ...
        if lang in QUOTES and type_ in QUOTES[lang]:
            quotes_list = QUOTES[lang][type_]
            quote = random.choice(quotes_list)
            safe_send(chat_id, f"📝 {quote}\n\nللمزيد، اكتب: more")
            remaining = [q for q in quotes_list if q != quote]
            user_states[f"{chat_id}_quotes"] = remaining
            user_states[f"{chat_id}_quote_type"] = f"{lang}_{type_}"
        return
...
def handle_text(message):
    ...
    if text.lower() == 'more' and user_states.get(f"{chat_id}_quotes"):
        remaining = user_states.get(f"{chat_id}_quotes", [])
        if remaining:
            to_send = remaining[:10]
            msg = "📝 اقتباسات إضافية:\n\n"
            for q in to_send:
                msg += f"- {q}\n"
            safe_send(chat_id, msg)
            user_states[f"{chat_id}_quotes"] = remaining[10:]
        else:
            safe_send(chat_id, "لا توجد اقتباسات إضافية")
        return
...
# ===================== تشغيل البوت (الإصلاح النهائي) =====================

if __name__ == '__main__':
    # إزالة أي webhook قديم لتجنب التعارض
    try:
        bot.remove_webhook()
        time.sleep(1)
    except:
    ...
    # بدء خيط Keep-Alive (يحافظ على نشاط الـ instance)
    threading.Thread(target=keep_alive, daemon=True).start()

    # تشغيل البوت باستخدام infinity_polling (يعيد المحاولة تلقائياً عند أي خطأ)
    # هذه هي الطريقة الأكثر استقراراً، ولا تحتاج لخيط إضافي لأنها تعمل في الخلفية
    def start_bot():
        try:
            # استخدام infinity_polling مع إعدادات محسّنة
            bot.infinity_polling(
                timeout=60,
                long_polling_timeout=60,
                skip_pending=True  # تجاهل التحديثات القديمة
            )
        except Exception as e:
            logging.error(f"Bot infinity_polling error: {e}")
            # في حالة حدوث خطأ غير متوقع، نعيد المحاولة بعد 10 ثوانٍ
            time.sleep(10)
            start_bot()  # إعادة تشغيل البوت

    # تشغيل البوت في خيط منفصل حتى لا يعيق تشغيل Flask
    threading.Thread(target=start_bot, daemon=True).start()

    # تشغيل Flask (يعمل في الخيط الرئيسي)
