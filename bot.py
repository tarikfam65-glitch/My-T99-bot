...
مع حل جذري لمشكلة الخمول وتحسين تقرير الثغرات
"""
...
SERVER_URL = os.environ.get('SERVER_URL', '')  # يجب تعيينه في Render إذا كنت تستخدم Webhook
...
USE_WEBHOOK = os.environ.get('USE_WEBHOOK', 'true').lower() == 'true'  # تفعيل Webhook إذا توفر SERVER_URL
...
if __name__ == "__main__":
    ...
    if USE_WEBHOOK and SERVER_URL:
        ...
        # استخدام Polling مع إضافة حماية ضد الخمول (keep-alive)
        logger.info("🔄 تشغيل البوت عبر Polling (مع آلية إبقاء النشاط)...")
        bot.remove_webhook()
        try:
            ...
            # بدء Polling مع timeout قصير لتجنب التجميد
