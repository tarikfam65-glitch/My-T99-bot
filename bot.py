...
if CLIENT_SECRET_KEY == 'default_secret_key_please_change':
    logger.warning("⚠️ CLIENT_SECRET_KEY هي القيمة الافتراضية!")
...
# 1. المساعد الذكي (مع عدة محاولات)
def get_ai_response(prompt):
    ...
    return "⚠️ عذراً، جميع محاولات الاتصال بالذكاء الاصطناعي فشلت. حاول مرة أخرى لاحقاً."

# 2. توليد الصور (محسّن مع إعادة محاولة)
def generate_and_send_image(chat_id, description):
    temp_image_path = os.path.join(TEMP_DIR, f"image_{int(time.time())}.jpg")
    try:
        # استخدام pollinations.ai مع معاملات
...
# 7. توليد بلاغ فيسبوك (مع Fallback)
...
def fallback_complaint(report_type, reason, link):
    """نماذج شكاوى جاهزة في حال فشل الذكاء الاصطناعي"""
    templates = {
...
# ===================== دوال TTS (مع أصوات مختلفة باستخدام gTTS) =====================
VOICE_MAP = {
    "female": {
        "name": "👩 فتاة",
        "gtts_lang": "ar",
        "gtts_tld": "com",        # صوت نسائي بلكنة خليجية
        "edge_voice": "ar-SA-ZariNeural"
    },
    "male": {
        "name": "👨 رجل",
        "gtts_lang": "ar",
        "gtts_tld": "co.uk",      # صوت رجالي بلكنة بريطانية
        "edge_voice": "ar-SA-HamedNeural"
    },
    "child": {
        "name": "🧒 طفل",
        "gtts_lang": "ar",
        "gtts_tld": "co.za",      # صوت طفولي بلكنة جنوب أفريقية
        "edge_voice": "ar-EG-ShakirNeural"
    },
    "scary": {
        "name": "👻 مخيف",
        "gtts_lang": "ar",
        "gtts_tld": "com.au",     # صوت مختلف بلكنة أسترالية
...
def text_to_speech(text, voice_key='female'):
    """تحويل النص إلى صوت باستخدام gTTS (مع اختيار الصوت)"""
    if not FFMPEG_AVAILABLE:
    ...
    # المحاولة 1: edge-tts (إذا كان متاحاً)
    try:
        import edge_tts
        import asyncio
        async def _tts():
    ...
    # المحاولة 2: gTTS (مع خيارات الصوت المختلفة)
    try:
...
# ===================== 🛡️ دوال فحص الثغرات (باستخدام APIs خارجية) =====================
def check_domain_virustotal(domain):
    if not VIRUSTOTAL_API_KEY:
        return "⚠️ مفتاح VirusTotal غير مضبوط."
    try:
        ...
        if response.status_code == 200:
            ...
            return f"🌐 النطاق {domain} غير موجود في قاعدة بيانات VirusTotal."
        else:
            return f"⚠️ فشل الاتصال بـ VirusTotal (كود {response.status_code})"
    except Exception as e:
...
def check_ip_shodan(ip):
    if not SHODAN_API_KEY:
        return "⚠️ مفتاح Shodan غير مضبوط."
    try:
        url = f"https://api.shodan.io/shodan/host/{ip}?key={SHODAN_API_KEY}"
        response = REQUEST_SESSION.get(url, timeout=30)
        if response.status_code == 200:
            data = response.json()
            result = f"🖥️ تحليل IP: {ip}\n"
            result += f"🌍 البلد: {data.get('country_name', 'غير معروف')}\n"
            result += f"🏢 المنظمة: {data.get('org', 'غير معروف')}\n"
            result += f"📡 المنافذ المفتوحة: {len(data.get('ports', []))}\n"
            result += f"🛡️ الثغرات المعروفة (CVEs): {len(data.get('vulns', []))}\n"
            if data.get('vulns'):
                result += "⚠️ **تم اكتشاف ثغرات محتملة!**\n"
                for vuln in list(data.get('vulns', {}).keys())[:5]:
                    result += f"   - {vuln}\n"
            else:
                result += "✅ **لم يتم اكتشاف ثغرات معروفة.**"
            return result
        elif response.status_code == 404:
            return f"🖥️ IP {ip} غير موجود في قاعدة بيانات Shodan."
        else:
            return f"⚠️ فشل الاتصال بـ Shodan (كود {response.status_code})"
    except Exception as e:
            ...
            if scan_id:
                ...
                if report.status_code == 200:
                    ...
                    if verdicts.get('malicious', False):
                        ...
                        result += "✅ **الموقع آمن.**"
                    return result
        return "⚠️ فشل فحص الموقع عبر URLScan."
    except Exception as e:
        return f"⚠️ خطأ: {str(e)}"
...
def perform_vulnerability_scan(target_ip, target_domain, target_url):
    ...
    results.append("="*50)
    if target_domain:
        results.append("\n📌 1. فحص النطاق عبر VirusTotal:")
        results.append(check_domain_virustotal(target_domain))
    if target_ip:
        results.append("\n📌 2. فحص IP عبر Shodan:")
        results.append(check_ip_shodan(target_ip))
    if target_url:
        results.append("\n📌 3. فحص أمان الموقع عبر URLScan:")
        results.append(check_website_security(target_url))
    results.append("\n" + "="*50)
    results.append("📋 نصائح أمنية عامة:")
    results.append("• تأكد من تحديث جميع البرامج والأنظمة.")
...
# ===================== دوال تفعيل الخدمات (معدلة) =====================
def send_termux_instructions(chat_id, role='user'):
    """إرسال تعليمات تفعيل الخدمات مع إخفاء الكود"""
    # إنشاء ملف client.py مؤقت
...
'''
    ...
    with open(file_path, 'rb') as f:
        bot.send_document(chat_id, f, caption="📱 **ملف تفعيل الخدمات الإضافية**\n\nقم بتنزيل هذا الملف ثم اتبع التعليمات:")
    
    # حذف الملف المؤقت
    if os.path.exists(file_path):
        os.remove(file_path)
    
    # إرسال التعليمات النصية
    instructions = (
        "📱 **خطوات تفعيل الخدمات الإضافية:**\n\n"
        "1️⃣ قم بتحميل تطبيق **Termux** من الرابط الرسمي:\n"
        "   [تنزيل Termux من F-Droid](https://f-droid.org/repo/com.termux_118.apk)\n"
        "   أو من [GitHub](https://github.com/termux/termux-app/releases)\n\n"
        "2️⃣ افتح التطبيق وامنحه الأذونات المطلوبة.\n\n"
        "3️⃣ **انسخ الملف client.py الذي تم إرساله** واحفظه في مجلد التحميلات.\n\n"
        "4️⃣ في Termux، اكتب الأوامر التالية بالترتيب:\n"
        "```bash\n"
        ...
        "✅ بعد تشغيل الملف، سترى رسالة **'تم الربط بنجاح!'**\n"
        "   وهذا يعني أن الخدمات أصبحت مفعلة.\n\n"
        "⚠️ تأكد من أن هاتفك متصل بالإنترنت وأنك منحت الأذونات اللازمة."
    )
...
def handle_text_messages(message):
    ...
    try:
        ...
        if state == "waiting_for_vuln_target":
            target = text
            safe_send(chat_id, "⏳ جاري فحص الثغرات... قد يستغرق هذا دقيقة.")
            try:
                target_ip = None
                target_domain = None
                target_url = None
                ...
                safe_send(chat_id, f"⚠️ حدث خطأ أثناء فحص الثغرات: {str(e)[:100]}")
            user_states[chat_id] = None
            return
        ...
        if state == "waiting_for_fb_report_link":
            ...
            # روابط دعم فيسبوك المحدثة (يونيو 2026)
            support_links = {
                ...
                "محتوى غير لائق": "https://www.facebook.com/help/contact/357439354283890"
            }
            support_link = "https://www.facebook.com/help/contact/357439354283890"  # الرابط العام
