...
def get_gemini_response(user_text):
    try:
        ...
        return "⚠️ حدث خطأ في Gemini: قد يكون المفتاح غير صحيح أو أن الخدمة مشغولة. حاول مرة أخرى لاحقاً."
...
def text_to_speech(text, lang='ar'):
    if not FFMPEG_AVAILABLE:
        return "FFmpeg غير مثبت على النظام. يرجى تثبيته لتشغيل هذه الميزة."
...
def build_main_menu(user_id):
    ...
    # الأزرار المغلقة (🔒) للمستخدم العادي: فحص الأمان، فحص التطبيقات، مراجعة الكود، تتبع رقم، فحص التسريبات، تواصل اجتماعي، نص صوت، الحصول على رقم
    # باقي الأزرار مفتوحة (بدون قفل)
    ...
    markup.row(
        ...
        InlineKeyboardButton("🧠 مساعد ذكي", callback_data="mode_ai")  # مفتوح
    )
    
    # الصف الثالث
    markup.row(
        InlineKeyboardButton("🎨 توليد صور", callback_data="mode_image"),  # مفتوح (يستهلك نقاط عند الاستخدام)
    ...
    # الصف التاسع
    markup.row(
        InlineKeyboardButton("🔊 نص صوت 🔒" if not is_admin(user_id) else "🔊 نص صوت ✅", 
...
def handle_text_messages(message):
    ...
    try:
        ...
        if state == "waiting_for_tts":
            ...
            result = text_to_speech(text, lang='ar')
            if isinstance(result, str) and result.startswith("FFmpeg"):
                safe_send(chat_id, f"⚠️ {result}")
