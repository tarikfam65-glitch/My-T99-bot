...
"""
ShadowNet Framework v9.5 - النسخة النهائية المستقرة بالكامل
- تم حذف import builtwith (غير مستخدم)
- تم حذف num_threads من polling
- دوال قاعدة البيانات مؤمنة (is_banned وغيرها)
- إضافة delete_webhook قبل polling
- تحسين Keep-Alive مع Ping داخلي
- المحتوى الإسلامي مدمج (JSON)
- جاهز للتشغيل على Render
"""
...
try:
    ...
    # تم حذف import builtwith لأنه غير مستخدم ويسبب أخطاء
    import googletrans
...
def start_bot():
    while True:
        try:
            ...
            # تم حذف num_threads لأنه غير مدعوم
            bot.polling(
                none_stop=True,
                interval=0,
                timeout=60,
