...
# ===================== دوال قاعدة البيانات =====================
# (جميع دوال قاعدة البيانات كما هي - تم حذفها للاختصار لأنها موجودة سابقاً)
...
def get_ai_response(prompt):
    ...
    # المحاولة 3: Gemini API (إذا كان المفتاح متاحاً)
    if GEMINI_API_KEY:
        try:
    ...
    # المحاولة 4: ردود افتراضية ذكية (إذا فشل كل شيء)
    fallback_responses = [
        "أنا آسف، ولكنني لم أتمكن من الاتصال بالخادم. يرجى المحاولة مرة أخرى لاحقاً.",
        "حدث خطأ في الاتصال. الرجاء التأكد من اتصال الإنترنت والمحاولة مجدداً.",
        "آسف، الخدمة غير متاحة حالياً. حاول في وقت لاحق."
    ]
...
def generate_and_send_image(chat_id, description):
    ...
    try:
        ...
        if response.status_code != 200 or not response.content:
            # محاولة بديلة: image.pollinations.ai
...
اكتبها بشكل احترافي وواضح."""
    ...
    if "آسف" in ai_response or "خطأ" in ai_response:
...
# ===================== دوال TTS مع أصوات مختلفة باستخدام edge-tts =====================
VOICE_MAP = {
    "female": {
        "name": "👩 صوت أنثوي",
...
def text_to_speech(text, voice_key='female'):
    ...
    # المحاولة 2: gTTS (مع اختلاف tld لتحقيق اختلاف بسيط في الصوت)
    try:
