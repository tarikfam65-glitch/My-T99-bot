...
"""
ShadowNet Framework v8.3 - إصدار مستقر مع تحسينات كاملة
تم إزالة ftplib من المتطلبات، وحذف ميزة السب، وإصلاح الاقتباسات، وتطوير التصيد والطقس
...
def get_weather_detailed(city):
    ...
    try:
        # استخدام wttr.in مع تنسيق JSON للحصول على تفاصيل أكثر
...
def callback_handler(call):
    ...
    if data == "phishing_email":
        user_states[chat_id] = "waiting_phishing_email_platform"
        safe_send(chat_id, "اختر المنصة التي تريد إنشاء بريد تصيد لها:\n(فيسبوك، انستغرام، تيك توك، تويتر)")
        return
...
def handle_text(message):
    ...
    # ===== بريد تصيد =====
    if state == "waiting_phishing_email_platform":
        platform = text.lower()
        if platform not in ['facebook', 'instagram', 'tiktok', 'twitter']:
            safe_send(chat_id, "منصة غير مدعومة. اختر: فيسبوك، انستغرام، تيك توك، تويتر")
            return
    ...
    if state == "waiting_phishing_email_target":
        ...
        safe_send(chat_id, "أدخل موضوع البريد (أو اكتب 'اقتراح' للحصول على موضوع مقترح):")
        return
    if state == "waiting_phishing_email_subject":
        if text.lower() == 'اقتراح':
            suggestions = [
                "تنبيه أمني: تم اكتشاف نشاط غير عادي على حسابك",
                "تحديث سياسة الخصوصية: يرجى مراجعة حسابك",
                "طلب إعادة تعيين كلمة السر",
                "تأكيد هوية المستخدم",
                "تحذير: تم محاولة دخول غير مصرح به"
            ]
            subject = random.choice(suggestions)
            safe_send(chat_id, f"تم اقتراح الموضوع: {subject}")
        else:
            subject = text
        user_states[chat_id] = "waiting_phishing_email_body"
        user_states[f"{chat_id}_phishing_subject"] = subject
    ...
    if state == "waiting_phishing_email_body":
        ...
        result = send_phishing_email(target_email, subject, body, phishing_link, platform)
        if result is True:
            safe_send(chat_id, f"✅ تم إرسال بريد التصيد إلى {target_email} بنجاح")
        else:
            safe_send(chat_id, f"❌ فشل إرسال البريد: {result}")
        user_states[chat_id] = None
        for key in [f"{chat_id}_phishing_target", f"{chat_id}_phishing_subject", f"{chat_id}_phishing_platform"]:
    ...
    if state == "waiting_lock_chat":
        # تم استبداله بقائمة المستخدمين، نترك هذه للمعالجة النصية القديمة
        pass
