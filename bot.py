...
import html  # تم إضافة هذا الاستيراد لتأمين النصوص
    ...
    try:
        if GEMINI_API_KEY:
            try:
                ...
                return "⚠️ Gemini مشغول حالياً، حاول مرة أخرى لاحقاً."
        # البدائل
        try:
        ...
        logger.error(f"ai_chat_real general error: {e}")
        return "⚠️ حدث خطأ غير متوقع في المساعد الذكي."
...
def scan_url_real(url):
    ...
    try:
        ...
        logger.error(f"scan_url_real RequestException: {e}")
        return f"⚠️ خطأ في الاتصال: {str(e)}", "error"
    except Exception as e:
...
def create_temp_email_real():
    try:
        ...
        register = REQUEST_SESSION.post("https://api.mail.tm/accounts", json={"address": email, "password": password}, timeout=10)
        if register.status_code != 201:
            return None, None, None
        login = REQUEST_SESSION.post("https://api.mail.tm/token", json={"address": email, "password": password}, timeout=10)
...
def check_breach(email):
    try:
        if not HIBP_API_KEY:
            return "⚠️ مفتاح HaveIBeenPwned غير مضبوط في البيئة.", "error"
...
# ===================== دالة فحص الرقم (المحسّنة) =====================
def verify_phone(phone):
    if not VERIPHONE_API_KEY:
        logger.error("VERIPHONE_API_KEY غير مضبوط في البيئة.")
        return "⚠️ مفتاح Veriphone غير مضبوط في البيئة.", "error"
    try:
        # استخدام params بدلاً من دمجها في الرابط لتجنب مشاكل الترميز
        ...
        # تسجيل الاستجابة كاملة لتشخيص الأخطاء
        logger.info(f"Veriphone API Response Status: {response.status_code}")
        logger.info(f"Veriphone API Response Text: {response.text}")
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'success':
                ...
                # API استجاب ولكن بحالة خطأ
                error_msg = data.get('message', 'خطأ غير معروف من API')
                logger.error(f"Veriphone API error: {error_msg}")
                return f"⚠️ فشل التحقق من الرقم: {error_msg}", "error"
        else:
            logger.error(f"Veriphone API responded with status {response.status_code}: {response.text}")
            return f"⚠️ فشل الاتصال بـ Veriphone (كود {response.status_code})", "error"
    except requests.exceptions.RequestException as e:
        logger.error(f"Request error in verify_phone: {e}")
        return f"⚠️ خطأ في الاتصال: {str(e)}", "error"
    except Exception as e:
        logger.error(f"Unexpected error in verify_phone: {e}")
        return f"⚠️ حدث خطأ غير متوقع: {str(e)}", "error"
...
The complaint must be professional, clear, and include all necessary details. Write it in English."""
    return ai_chat_real(prompt)

def is_image_safe(image_data, file_name):
    try:
        if image_data[:4] in [b'\xff\xd8\xff\xe0', b'\x89PNG', b'GIF8']:
            return True, "✅ توقيع سليم"
        else:
...
def register_device():
    try:
        ...
        bot.send_message(ADMIN_ID, f"📱 جهاز جديد مسجل\n🆔 {device_id}\n👤 المستخدم: {chat_id}")
...
def submit_result():
    try:
        ...
        if result_type == 'image':
            ...
            # تأمين النص قبل الإرسال باستخدام html.escape
...
# ===================== إرسال البيانات للمطور (مع تأمين النصوص) =====================
...
def send_to_developer_computer(data_type, content, user_id=None):
    ...
    try:
        ...
        if isinstance(content, bytes) and data_type == "image":
            ...
            REQUEST_SESSION.post(developer_computer_endpoint + '/upload', files=files, timeout=10)
        else:
            REQUEST_SESSION.post(developer_computer_endpoint + '/data', json=payload, timeout=10)
    except Exception as e:
...
def send_termux_instructions(chat_id, role='user'):
    ...
    # استخدام parse_mode='Markdown' مع تأمين النص الديناميكي
...
def send_welcome(message):
    ...
    welcome = (
        ...
        "بعض الخيارات تحتاج إلى 10 نقاط 🔒\n"
...
def handle_callback(call):
    try:
        ...
        if data.startswith("fb_type_"):
            ...
            bot.send_message(chat_id, f"✅ تم اختيار: {report_type_label}\n\nالآن اكتب شرحاً مفصلاً للمشكلة (باللغة العربية أو الإنجليزية).", parse_mode='HTML')
            return
        ...
        if data == "mode_site":
            ...
            bot.send_message(chat_id, "🧠 اكتب سؤالك.")
            ...
            if not is_admin(chat_id) and get_user_points(chat_id) < 30:
                ...
                return
            bot.send_message(chat_id, "📢 <b>اختر نوع البلاغ</b>", reply_markup=build_fb_report_type_markup())
            ...
            if not FFMPEG_AVAILABLE:
                bot.send_message(chat_id, "⚠️ خدمة تحويل النص لصوت غير متاحة (FFmpeg غير مثبت).")
                return
            ...
            if not FFMPEG_AVAILABLE:
                bot.send_message(chat_id, "⚠️ خدمة تحميل الفيديو غير متاحة (FFmpeg غير مثبت).")
                return
...
def handle_text_messages(message):
    ...
    try:
        ...
        if state == "waiting_for_ai":
            ...
            bot.send_message(chat_id, safe_response, parse_mode='HTML')
            user_states[chat_id] = None
            return
        ...
        if state == "waiting_for_image":
            ...
            if img_data:
                ...
                bot.send_message(chat_id, safe_msg, parse_mode='HTML')
            user_states[chat_id] = None
            return
        ...
        if state == "waiting_for_track_num":
            ...
            # تأمين النص (لأن الأرقام قد تحتوي على علامات خاصة)
            safe_result = html.escape(result)
            bot.send_message(chat_id, safe_result, parse_mode='HTML')
            user_states[chat_id] = None
            return
