...
"""
ShadowNet Framework v7.7 - النظام المتكامل للاختبارات الأمنية
تم إعادة زر تحميل الفيديو وإصلاح جميع المشاكل السابقة
...
def handle_text(message):
    ...
    # إذا كان المستخدم محظوراً، لا نرسل رسالة "الدردشة مقفلة" بل نتعامل بصمت
    if is_banned(chat_id) and not is_admin(chat_id):
        # لا نرسل أي شيء، فقط نسجل النشاط
        return
    ...
    if state is None:
        # إذا لم يكن محظوراً، نعرض القائمة
        safe_send(chat_id, "اختر خدمة من القائمة:", reply_markup=build_main_menu(chat_id))
...
if __name__ == '__main__':
    ...
    # هذه هي الطريقة الأكثر استقراراً
    def start_bot():
        try:
