# ===================== bot.py (النسخة النهائية - مع تعيين Webhook تلقائي) =====================
# -*- coding: utf-8 -*-
...
# ===================== تشغيل البوت (مع تعيين Webhook تلقائي) =====================
...
def start_bot():
    ...
    if USE_WEBHOOK:
        # تعيين Webhook قبل تشغيل الخادم
        if not set_webhook():
            logger.warning("⚠️ فشل تعيين Webhook، سيتم استخدام Polling كحل احتياطي.")
            # استمرار التشغيل باستخدام Polling
            ...
            # تشغيل Flask كعملية رئيسية
            flask_app.run(host='0.0.0.0', port=PORT, threaded=True)
    else:
        # تشغيل Flask في خلفية واستخدام Polling
        flask_thread = threading.Thread(target=run_flask, daemon=True)
        flask_thread.start()
        logger.info(f"✅ خادم Flask يعمل على المنفذ {PORT} (Polling mode)")
