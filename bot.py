...
"""
ShadowNet Framework v8.8 - النسخة النهائية المستقرة تماماً
- إصلاح خطأ 409 Conflict مع إعادة محاولة ذكية
- إضافة ensure_user_exists لمنع أخطاء قاعدة البيانات
- جميع الأزرار تعمل: الطقس، ويكيبيديا، مولد كلمات سر، فحص القوة، تحويل نص لصوت، ترجمة، تذكير، أخبار، اقتباسات، نصائح، فحص الرابط، تحليل APK، تحليل PDF، نقاط، إحالات، فيديو، تصيد، بريد تصيد، أذكار الصباح والمساء، أدعية
- نظام صلاحيات متقدم لكل زر
- صورة ترحيبية جديدة
- تحمل الضغط وإعادة تشغيل تلقائي مع سجلات تفصيلية
"""
...
def ensure_user_exists(chat_id):
    """تأكد من وجود المستخدم في قاعدة البيانات، أنشئه إذا لم يكن موجوداً."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def build_main_menu(chat_id):
    try:
        ...
        # قائمة افتراضية بسيطة في حال الفشل
...
def process_text(message):
    try:
        ...
        # تأكد من وجود المستخدم في قاعدة البيانات فوراً
        ensure_user_exists(chat_id)
...
if __name__ == '__main__':
    print("✅ ShadowNet v8.8 بدأ التهيئة...")
