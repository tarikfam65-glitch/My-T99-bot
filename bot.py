...
يعمل بـ Polling مع دعم Webhook كخيار احتياطي
"""
...
if not TELEGRAM_TOKEN:
    logger.critical("❌ TELEGRAM_TOKEN غير مضبوط! البوت لن يعمل.")
    sys.exit(1)

ADMIN_ID = int(os.environ.get('ADMIN_ID', 0))
if ADMIN_ID == 0:
    logger.critical("❌ ADMIN_ID غير مضبوط! البوت لن يعمل.")
    sys.exit(1)
...
if CLIENT_SECRET_KEY == 'default_secret_key_please_change':
    logger.warning("⚠️ تحذير: CLIENT_SECRET_KEY هي القيمة الافتراضية! قم بتغييرها.")
...
VOICE_MAP = {
    "female": {
        "name": "👩 فتاة",
        "voice": "ar-SA-ZariNeural",      # صوت نسائي سعودي
        "lang": "ar",
        "tld": "com"
    },
    "male": {
        "name": "👨 رجل",
        "voice": "ar-SA-HamedNeural",     # صوت رجالي سعودي
        "lang": "ar",
        "tld": "com"
    },
    "child": {
        "name": "🧒 طفل",
        "voice": "ar-EG-ShakirNeural",    # صوت طفولي مصري
        "lang": "ar",
        "tld": "com"
    },
    "scary": {
        "name": "👻 مخيف",
        "voice": "ar-AE-FatimaNeural",    # صوت إماراتي - يمكن تعديله
        "lang": "ar",
        "tld": "com"
    }
}
...
def text_to_speech(text, voice_key='female'):
    """تحويل النص إلى صوت باستخدام edge-tts مع اختيار الصوت"""
    if not FFMPEG_AVAILABLE:
    ...
    try:
        ...
        # ✅ البديل: استخدام gTTS مع اختيار الصوت
        try:
            from gtts import gTTS
            tts = gTTS(text=clean_text, lang=voice["lang"], tld=voice["tld"], slow=False)
            tts.save(temp_file)
            if os.path.exists(temp_file) and os.path.getsize(temp_file) > 0:
                return temp_file, None
            else:
