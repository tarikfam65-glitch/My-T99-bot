...
# 1. Gemini AI (تم تغيير النموذج إلى gemini-1.5-pro-latest)
def get_gemini_response(user_text):
    try:
        if not GEMINI_API_KEY:
            return "⚠️ مفتاح Gemini غير مضبوط. يرجى إضافة GEMINI_API_KEY."
        import google.generativeai as genai
        genai.configure(api_key=GEMINI_API_KEY)
        # ✅ استخدام أحدث نموذج مستقر
        ...
        return f"⚠️ حدث خطأ في Gemini: {str(e)[:100]}... حاول مرة أخرى."

# 2. توليد الصور (Pollinations AI - مجاني)
def generate_image_real(description):
    try:
        url = f"https://pollinations.ai/p/{description.replace(' ', '%20')}"
...
# 3. فحص الروابط (VirusTotal API مع fallback)
def scan_url_real(url):
    url = url.strip()
    # المحاولة الأولى: VirusTotal
    if VIRUSTOTAL_API_KEY:
        try:
            scan_url = "https://www.virustotal.com/api/v3/urls"
            headers = {"x-apikey": VIRUSTOTAL_API_KEY}
            data = {"url": url}
            response = REQUEST_SESSION.post(scan_url, headers=headers, data=data, timeout=30)
            if response.status_code == 200:
                ...
                if scan_id:
                    ...
                    if report.status_code == 200:
                        ...
                        if malicious > 0:
                            return f"🚨 تم اكتشاف {malicious} تهديدات في الرابط!", "malicious"
                        elif suspicious > 0:
                            return f"⚠️ تم اكتشاف {suspicious} عناصر مشبوهة.", "suspicious"
                        else:
                            return "✅ الرابط آمن.", "safe"
        except Exception as e:
...
def check_breach(email):
    try:
        if not HIBP_API_KEY:
            return "⚠️ مفتاح HaveIBeenPwned غير مضبوط. يرجى إضافة HIBP_API_KEY", "error"
...
def verify_phone(phone):
    if not VERIPHONE_API_KEY:
        return "⚠️ مفتاح Veriphone غير مضبوط. يرجى إضافة VERIPHONE_API_KEY", "error"
    try:
...
# ===================== دوال TTS مع خيارات الأصوات =====================
VOICE_OPTIONS = {
    "female": {"name": "👩 فتاة", "lang": "ar", "tld": "com", "slow": False},
    "male": {"name": "👨 رجل", "lang": "ar", "tld": "com.eg", "slow": False},
    "child": {"name": "🧒 طفل", "lang": "ar", "tld": "sa", "slow": False},
    "slow": {"name": "🐢 بطيء", "lang": "ar", "tld": "com", "slow": True}
}
...
def text_to_speech(text, voice_key='female'):
    """تحويل النص إلى صوت باستخدام gTTS مع خيارات الصوت"""
    if not FFMPEG_AVAILABLE:
        return None, "⚠️ FFmpeg غير مثبت. يرجى تثبيته."
    
    voice = VOICE_OPTIONS.get(voice_key, VOICE_OPTIONS['female'])
...
linked_users = set()
user_welcome_sent = {}
tts_voice_selection = {}  # لتخزين اختيار الصوت مؤقتاً
...
def check_permission(user_id, feature_type='locked', required_points=10):
    if is_admin(user_id):
        return True, 0
...
def build_main_menu(user_id):
    ...
    markup.row(
        InlineKeyboardButton("🖼️ إنشاء صور", callback_data="mode_image"),  # تم التغيير
        InlineKeyboardButton("📤 انشاء بريد", callback_data="mode_temp_email")  # تم التغيير
    )
    if is_admin_user or user_points >= 30:
        markup.row(
            InlineKeyboardButton("بلاغ فيسبوك" if is_admin_user else "بلاغ فيسبوك ✅",  # تم التغيير
                                callback_data="mode_fb_report"),
            InlineKeyboardButton("🔎 فحص الأرقام", callback_data="mode_spam_block")  # تم التغيير
        )
    else:
        markup.row(
            InlineKeyboardButton("بلاغ فيسبوك 🔒", callback_data="locked_fb_report"),
            InlineKeyboardButton("🔎 فحص الأرقام", callback_data="mode_spam_block")
        )
    markup.row(
        ...
        InlineKeyboardButton("🛡️ فحص التسريبات" if is_admin_user else ("🛡️ فحص التسريبات ✅" if user_points >= 10 else "🛡️ فحص التسريبات 🔒"), 
                            callback_data="mode_fb_hacked" if is_admin_user or user_points >= 10 else "locked_fb_hacked")
    )
    markup.row(
    ...
    markup.row(
        InlineKeyboardButton("🔊 تحويل نص إلى صوت" if is_admin_user else ("🔊 تحويل نص إلى صوت ✅" if user_points >= 10 else "🔊 تحويل نص إلى صوت 🔒"),  # تم التغيير
    ...
    markup.row(
        InlineKeyboardButton("🔗 تقصير الروابط", callback_data="mode_shorten_url")  # زر جديد
    )
...
def help_command(message):
    help_text = (
        ...
        "🔒 الأزرار المغلقة تتطلب نقاطاً (10 نقاط للاستخدام).\n"
        "🔓 الأزرار المفتوحة متاحة للجميع (بعضها يستهلك نقاطاً عند الاستخدام).\n"
        "👑 المطور لديه صلاحية كاملة على جميع الأزرار ولا يحتاج نقاط."
    )
...
# ===================== معالج النقر على الأزرار (مع إصلاح 400 Bad Request) =====================
@bot.callback_query_handler(func=lambda call: True)
def handle_callback(call):
    # ✅ الخطوة الأولى: إغلاق الإشعار فوراً لتجنب 400 Bad Request
    try:
    ...
    # ===== الأزرار المغلقة التي تتطلب نقاطاً =====
    ...
    # ===== المطور: مساعد ذكي وإنشاء صور بدون نقاط =====
    if data == "mode_ai":
        if is_admin(chat_id):
    ...
    try:
        ...
        if state == "waiting_for_shorten":
            if not re.match(r'^https?://', text):
                safe_send(chat_id, "❌ رابط غير صالح. تأكد من أن الرابط يبدأ بـ http:// أو https://")
                return
        ...
        if state == "waiting_for_image":
            ...
            if img_data:
                bot.send_photo(chat_id, img_data, caption="🖼️ الصورة المولدة")
            else:
