...
def handle_callback(call):
    try:
        ...
        # ===== ✅ إصلاح المطور: توليد صور ومساعد ذكي =====
        if data == "mode_ai":
            if is_admin(chat_id):
                # المطور: لا يتم خصم نقاط
        ...
        if data == "mode_image":
            if is_admin(chat_id):
                ...
                safe_send(chat_id, "🎨 اكتب وصف الصورة (المطور معفي من خصم النقاط).")
                return
            # المستخدم العادي: تحقق من النقاط
            points = get_user_points(chat_id)
            if points < 10:
                safe_send(chat_id, "❌ نقاط غير كافية! تحتاج 10 نقاط لتوليد صورة. ادعُ أصدقاءك لكسب المزيد.")
                return
