...
"""
ShadowNet Framework v14.0 - دمج نظام T11 بالكامل
- إضافة زر مساعد في القائمة الرئيسية
- نظام T11 بوجهين (عام + مطور)
- نظام احترام وتقارير
- صوت T11 بنبرات بشرية
"""
...
def t11_get_history(user_id, limit=10):
    try:
        conn = sqlite3.connect(T11_DB_PATH)
        c = conn.cursor()
...
# ===================== T11 - معالج المطور (Gemini) =====================
def t11_handle_dev(text, user_id):
    """معالج المطور باستخدام Gemini"""
    try:
        import google.generativeai as genai
        from langchain_google_genai import ChatGoogleGenerativeAI
        
        if not GEMINI_API_KEY:
            return "مفتاح Gemini غير مضبوط. يرجى إضافة GEMINI_API_KEY."
...
def t11_handle_public(text, user_id):
    """معالج المستخدمين العاديين باستخدام Gemini"""
    try:
        import google.generativeai as genai
        from langchain_google_genai import ChatGoogleGenerativeAI
...
# ===================== متغيرات T11 =====================
t11_user_mode = {}  # وضع المستخدم (assistant / dev)
t11_user_msg_count = {}  # عدد رسائل المستخدم
t11_user_voice_preference = {}  # تفضيل الصوت
...
def callback_handler(call):
    try:
        ...
        # ===== قائمة T11 =====
        if data == "t11_menu":
            safe_send(chat_id, "🤖 مرحباً بك في بروتوكول T11\nاختر خدمة من القائمة:", reply_markup=build_t11_menu())
            return

        if data == "t11_report":
            if chat_id != ADMIN_ID and chat_id not in [6688256875, 8080689721]:
            ...
            try:
                conn = sqlite3.connect(T11_DB_PATH)
                c = conn.cursor()
                c.execute("SELECT COUNT(DISTINCT user_id) FROM t11_memory")
                users = c.fetchone()[0]
                conn.close()
                reply += f"👥 مستخدمين نشطين: {users}"
            except:
                pass
...
• تم تطويره بواسطة المطور <b>T99</b>
• يستخدم تقنيات الذكاء الاصطناعي المتقدمة
• يتميز بشخصية فريدة ونظام احترام
• يدعم الأوامر الصوتية والنصية
• يعمل ببروتوكول ShadowNet

<b>الإصدار:</b> 2.0
<b>البروتوكول:</b> T11
"""
            safe_send(chat_id, about_text, parse_mode='HTML')
            return

        if data == "t11_help":
            help_text = """
🤖 <b>بروتوكول T11 - المساعدة</b>
...
"""
            safe_send(chat_id, help_text, parse_mode='HTML')
            return

        if data == "t11_voice_settings":
            safe_send(chat_id, "🎤 اختر الصوت المفضل لديك:", reply_markup=build_t11_voice_settings())
            return

        if data.startswith("t11_set_voice_"):
            voice_name = data.replace("t11_set_voice_", "")
            voice_code = T11_VOICE_CODES.get(voice_name, T11_DEFAULT_VOICE)
            t11_user_voice_preference[chat_id] = voice_code
            safe_send(chat_id, f"✅ تم اختيار صوت {voice_name} كصوتك المفضل.")
            return
    ...
    try:
        ...
        if text.startswith('/say '):
            ...
            if audio_file:
                try:
                    with open(audio_file, 'rb') as f:
                        bot.send_audio(chat_id, f, caption=f"🎤 T11 يقول: {voice_text[:50]}...")
                    os.remove(audio_file)
                except Exception as e:
                    safe_send(chat_id, f"❌ فشل إرسال الصوت: {str(e)[:100]}")
            else:
        ...
        if chat_id in t11_user_mode and t11_user_mode[chat_id] == "assistant":
            ...
            if t11_is_banned(chat_id):
                safe_send(chat_id, "⛔ تم تقييدك. عد بعد ساعة.")
                return
            ...
            if status == "ban":
                ...
                safe_send(chat_id, "⛔ تم تقييدك بسبب الإساءة المتكررة.")
                return
            if status == "warn":
                new_respect = t11_update_respect(chat_id, -30)
                safe_send(chat_id, f"⚠️ أسلوب مرفوض. تم خصم 30.\nالاحترام الحالي: {new_respect}")
                return
            if status == "dumb":
                new_respect = t11_update_respect(chat_id, -15)
                safe_send(chat_id, f"🤖 طلبك خارج نطاقي. تم خصم 15.\nالاحترام الحالي: {new_respect}")
                return
            ...
            # معالجة المطور
            if chat_id == ADMIN_ID or chat_id in [6688256875, 8080689721]:
                response = t11_handle_dev(text, chat_id)
                safe_send(chat_id, response)
                # إرسال الصوت
                voice_code = t11_user_voice_preference.get(chat_id, T11_DEFAULT_VOICE)
                audio_file = t11_generate_voice(response[:500], voice_code)
                if audio_file:
                    try:
                        with open(audio_file, 'rb') as f:
                            bot.send_audio(chat_id, f)
        ...
        # ===== T11 - طلب صوت =====
        if state == "t11_waiting_voice":
            user_states[chat_id] = None
            voice_code = t11_user_voice_preference.get(chat_id, T11_DEFAULT_VOICE)
            safe_send(chat_id, f"⏳ جاري تحويل النص إلى صوت...")
            audio_file = t11_generate_voice(text, voice_code)
            if audio_file:
                try:
                    with open(audio_file, 'rb') as f:
                        bot.send_audio(chat_id, f, caption=f"🎤 T11: {text[:50]}...")
                    os.remove(audio_file)
                except Exception as e:
                    safe_send(chat_id, f"❌ فشل إرسال الصوت: {str(e)[:100]}")
            else:
        ...
        if text.startswith('/start'):
            ...
            # تفعيل T11 للمستخدم
            t11_user_mode[chat_id] = "assistant"
            t11_user_msg_count[chat_id] = 0
