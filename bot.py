...
# 1. المساعد الذكي (مع خيارات متعددة)
def get_ai_response(prompt):
    """الحصول على رد من عدة مصادر مجانية"""
...
# 2. توليد الصور (مع تحسينات)
def generate_and_send_image(chat_id, description):
    """توليد صورة باستخدام Pollinations مع حفظ محلي"""
    ...
    try:
        ...
        if response.status_code != 200:
            safe_send(chat_id, f"⚠️ فشل توليد الصورة: خطأ في الاستجابة (كود {response.status_code})")
            return False
...
# ===================== دوال TTS (مع أصوات مختلفة مؤكدة) =====================
VOICE_MAP = {
    "female": {
        "name": "👩 فتاة",
...
def text_to_speech(text, voice_key='female'):
    ...
    # المحاولة 2: gTTS (مع اختيار اللغة واللهجة)
    try:
