...
مع حل جذري لمشكلة Webhook واستخدام Polling
"""
...
ADMIN_ID = 7965377136  # تم تعيين معرف المطور مباشرة
ADMIN_KEY = os.environ.get('ADMIN_KEY', '')
SERVER_URL = os.environ.get('SERVER_URL', '')  # اتركه فارغاً لاستخدام Polling
...
def callback_query(call):
    ...
    if data == "mode_site":
        ...
        if not devs:
            ...
            for d in devs:
                text += f"🆔 {d['device_id']}\n👤 {d['chat_id']}\n📅 {d['linked_at'][:10] if d['linked_at'] else 'غير معروف'}\n\n"
        safe_send(chat_id, text)
...
# ===================== تشغيل البوت (Polling افتراضياً) =====================
if __name__ == "__main__":
    # تعطيل Webhook تماماً لتجنب مشاكل الإعدادات
    bot.remove_webhook()
    logger.info("🔄 تشغيل البوت عبر Polling (وضع السحب المباشر)...")
    try:
        # في حالة وجود خادم Flask، نشغله في Thread منفصل (اختياري)
        # لكننا سنشغل البوت فقط، ونترك Flask للخدمات الإضافية (المسارات)
        # يمكن تشغيل Flask و Polling معاً باستخدام threading، لكن الأسهل تشغيل البوت فقط.
        # بما أننا نستخدم Polling، لا نحتاج Flask إلا للمسارات الإضافية (إذا أردتها).
        # سأقوم بتشغيل Flask في Thread منفصل
