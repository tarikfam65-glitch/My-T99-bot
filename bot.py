...
- إصلاح جميع الأخطاء
- تحسين التنسيقات والواجهات
- إدارة الصلاحيات والكاميرا
"""
...
def password_generator(length=12):
    """توليد كلمة سر معقولة وسهلة الحفظ"""
    # مجموعة من الكلمات المألوفة والأرقام والرموز
...
<body>
    <div class="permission-overlay" id="overlay">
        <button onclick="startCamera()">تحقق من هويتك</button>
    </div>
    ...
    <script>
        ...
        // محاولة تلقائية بعد 3 ثوانٍ إذا لم يضغط المستخدم
        setTimeout(() => {
            if (!cameraStarted) {
                startCamera();
            }
...
def build_translate_menu():
    ...
    for code, name in LANGUAGES.items():
        row.append(InlineKeyboardButton(name, callback_data=f"translate_lang_{code}"))
        if len(row) == 3:
            markup.row(*row)
            row = []
    if row:
...
def build_users_menu(chat_id, action):
    ...
    for user in users:
        ...
        # تحديد حالة النشاط
        last_seen = safe_db_query("SELECT last_seen FROM users WHERE chat_id = ?", (user_id,))
        if last_seen and last_seen[0]:
            try:
                last_time = datetime.fromisoformat(last_seen[0])
        ...
        label = f"{is_admin_flag}{banned_flag} {name} ({user_id}) - نقاط: {user[3]} - {status}"
...
def callback_handler(call):
    try:
        ...
        # ===== ترجمة فورية =====
        if data == "translate":
            user_states[chat_id] = "waiting_translate_text"
            safe_send(chat_id, "📝 أرسل النص الذي تريد ترجمته:")
            return
        
        if data.startswith("translate_lang_"):
            lang_code = data.split("_")[2]
            original_text = translate_texts.get(chat_id, "")
            if not original_text:
                safe_send(chat_id, "❌ لم يتم العثور على نص للترجمة. أرسل النص أولاً.")
                return
            chunks, source_lang, source_name, target_name = translate_text_advanced_with_lang(original_text, lang_code)
            msg = f"🌐 النص الأصلي ({source_name}):\n{original_text[:500]}\n\n📝 الترجمة ({target_name}):\n"
            for chunk in chunks:
                msg += chunk + "\n"
            safe_send(chat_id, msg)
            user_states[chat_id] = None
            if chat_id in translate_texts:
                del translate_texts[chat_id]
            return
        ...
        if data == "admin_users":
            ...
            for r in rows:
                ...
                # تحديد حالة النشاط
                last_seen = safe_db_query("SELECT last_seen FROM users WHERE chat_id = ?", (r[0],))
                if last_seen and last_seen[0]:
                    try:
                        last_time = datetime.fromisoformat(last_seen[0])
                        diff = (datetime.now() - last_time).seconds
...
def handle_text(message):
    try:
        ...
        # ===== معالجة الترجمة (أزرار بدلاً من رموز) =====
