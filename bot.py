...
باستخدام Groq API (للنصوص) و Hugging Face API (للصور)
"""
...
# ===== مفاتيح Groq و Hugging Face =====
GROQ_API_KEY = os.environ.get('GROQ_API_KEY', '')
HUGGINGFACE_API_KEY = os.environ.get('HUGGINGFACE_API_KEY', '')

if not GROQ_API_KEY:
    logger.warning("⚠️ GROQ_API_KEY غير مضبوط! المساعد الذكي لن يعمل.")
if not HUGGINGFACE_API_KEY:
    logger.warning("⚠️ HUGGINGFACE_API_KEY غير مضبوط! إنشاء الصور لن يعمل.")
...
if GROQ_API_KEY:
    try:
        ...
        logger.info("✅ Groq API تم تهيئته بنجاح")
...
# ===================== دوال الخدمات (باستخدام Groq و Hugging Face) =====================

# 1. الذكاء الاصطناعي (Groq)
def get_ai_response(prompt):
    """الحصول على رد من Groq API"""
    if not groq_client:
        return "⚠️ خدمة الذكاء الاصطناعي غير متاحة حالياً. يرجى إضافة GROQ_API_KEY."
    try:
        ...
        return f"⚠️ حدث خطأ في الذكاء الاصطناعي: {str(e)[:100]}... حاول مرة أخرى."

# 2. توليد الصور (Hugging Face)
def generate_image_huggingface(prompt):
    """توليد صورة باستخدام Hugging Face API"""
    if not HUGGINGFACE_API_KEY:
    ...
    try:
        response = REQUEST_SESSION.post(API_URL, headers=HEADERS, json=payload, timeout=60)
        if response.status_code == 200:
            return response.content, None
        else:
            logger.error(f"Hugging Face error: {response.status_code} - {response.text[:200]}")
            return None, f"⚠️ فشل توليد الصورة: {response.status_code}"
    except Exception as e:
        logger.error(f"Hugging Face error: {e}")
        return None, f"⚠️ خطأ: {str(e)[:100]}"

def generate_and_send_image(chat_id, description):
    """توليد صورة وحفظها محلياً ثم إرسالها مع تنظيف الملف"""
    temp_image_path = os.path.join(TEMP_DIR, f"image_{int(time.time())}.jpg")
    try:
        # توليد الصورة عبر Hugging Face
...
# 7. توليد بلاغ فيسبوك (باستخدام Groq)
...
def handle_callback(call):
    # ✅ إغلاق الإشعار فوراً لمنع 400 Bad Request
    try:
...
def handle_text_messages(message):
    ...
    try:
        ...
        # ===== إنشاء الصور (باستخدام Hugging Face) =====
