...
# ===================== إعداد Webhook (مع إعادة تعيين عند بدء التشغيل) =====================
def set_webhook():
    if not SERVER_URL:
    ...
    try:
        ...
        if success:
            logger.info(f"✅ Webhook تم تعيينه بنجاح: {webhook_full_url}")
            return True
        else:
...
# ===================== تشغيل التطبيق (تم الفصل بين Webhook و Polling) =====================
if __name__ == "__main__":
    if USE_WEBHOOK and SERVER_URL:
        logger.info("🔄 تشغيل البوت عبر Webhook...")
        # تأكد من تعيين الـ Webhook مرة واحدة عند التشغيل
        if set_webhook():
            logger.info(f"🚀 تشغيل خادم Flask على المنفذ {PORT}")
            # تشغيل خادم Flask فقط لاستقبال طلبات Webhook (بدون Polling)
            app.run(host='0.0.0.0', port=PORT, debug=False)
        else:
            logger.warning("⚠️ فشل تعيين Webhook، التحول إلى وضع Polling الاحتياطي...")
            bot.remove_webhook()
