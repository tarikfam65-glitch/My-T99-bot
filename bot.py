...
def webhook():
    try:
        if request.headers.get('content-type') == 'application/json':
            ...
            # إضافة سجل توثيق للتأكد
            logger.info(f"رسالة جديدة وصلت: {update.update_id}")
            
            # المعالجة الأساسية
            bot.process_new_updates([update])
            
            # الحل: التأكد من الرد المباشر إذا كان هناك نص
            if update.message:
                chat_id = update.message.chat.id
                # هذا السطر للرد فوراً على أي رسالة للتأكد من أن البوت "حي"
                bot.send_message(chat_id, "تم استلام رسالتك بنجاح!")
                logger.info(f"تم إرسال رد تجريبي إلى: {chat_id}")
...
def safe_send(chat_id, text, reply_markup=None):
    try:
        # استخدام parse_mode='HTML' لتجنب مشاكل التنسيق
...
def callback_query(call):
    ...
    # حالة اختيار صوت (محذوف لكن نحتفظ بها)
...
if __name__ == "__main__":
    # التأكد من تعيين الويب هوك إذا كان مطلوباً
    if USE_WEBHOOK and SERVER_URL:
        logger.info("🔄 تشغيل البوت عبر Webhook...")
        set_webhook()
        # تشغيل سيرفر Flask فقط (بدون polling)
        app.run(host='0.0.0.0', port=PORT, debug=False)
    else:
        # إذا لم يتم تعيين ويب هوك، يعمل البوت بنظام السحب المباشر
