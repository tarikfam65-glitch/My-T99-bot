...
# 1. Gemini AI (باستخدام gemini-1.5-flash)
...
# 2. توليد الصور (Pollinations AI - رابط مباشر)
...
def generate_image_real(description):
    try:
        image_url = generate_image_pollinations(description)
        # تحميل الصورة للتأكد من وجودها
        response = REQUEST_SESSION.get(image_url, timeout=30)
...
# 3. فحص الموقع (باستخدام requests + BeautifulSoup)
def scan_website(url):
    """فحص الموقع باستخدام requests و BeautifulSoup"""
    url = url.strip()
    try:
        ...
        return "⚠️ انتهت مهلة الاتصال بالموقع.", "error"
    except requests.exceptions.ConnectionError:
        return "⚠️ فشل الاتصال بالموقع. تأكد من الرابط.", "error"
    except Exception as e:
...
# 7. فحص الأرقام (Veriphone) - تم حذفها ولكن تركنا الدالة للاستخدام الداخلي
...
# 8. توليد بلاغ فيسبوك (مع Fallback)
...
Write it professionally and clearly in English."""
        ...
        logger.error(f"FB Report Gemini error: {e}")
        return fallback_complaint(report_type, reason, link)

def fallback_complaint(report_type, reason, link):
    """نص شكوى احتياطي في حال فشل Gemini"""
    return f"""Dear Facebook Support Team,

I am writing to formally report a violation of Facebook's Community Standards regarding {report_type}.
...
This content/account violates Facebook's policies and I kindly request that you review this matter and take appropriate action to ensure the safety and integrity of the platform.
...
# ===================== دوال TTS (مع Voice Mapping و تنظيف النص) =====================

# خريطة الأصوات لـ edge-tts
VOICE_MAP = {
...
def clean_text_for_tts(text):
    """تنظيف النص من الروابط والإيموجي والرموز غير الضرورية"""
...
def text_to_speech(text, voice_key='female'):
    """تحويل النص إلى صوت باستخدام edge-tts مع Voice Mapping"""
    if not FFMPEG_AVAILABLE:
...
def build_main_menu(user_id):
    ...
    # صف 7: تفعيل الخدمات + حماية إضافية
    markup.row(
    ...
    # صف 8: تحويل نص إلى صوت (مغلق) + تحميل فيديو
    markup.row(
    ...
    # صف 11: نقاطي + سجل النقاط + رابط دعوتي
    markup.row(
...
def handle_callback(call):
    # ✅ إصلاح 400 Bad Request: إغلاق الإشعار فوراً
    try:
