# ===================== bot.py (النسخة النهائية - مع Gemini API) =====================
# -*- coding: utf-8 -*-
...
# ===================== دوال الخدمات المحسنة (مع Gemini) =====================

def ai_chat_real(prompt):
    """دردشة ذكية باستخدام Gemini API أولاً، ثم APIs أخرى كحل احتياطي"""
    ...
    # المحاولة الرابعة: SimSimi (مجاني بدون مفتاح)
    try:
        sim_url = f"https://api.simsimi.vn/v1/simtalk?text={requests.utils.quote(prompt)}&lc=ar"
        sim_response = requests.get(sim_url, timeout=20)
        if sim_response.status_code == 200:
            sim_data = sim_response.json()
            if sim_data and 'message' in sim_data:
                return sim_data['message']
    except Exception as e:
...
def generate_image_real(description):
    """توليد صورة باستخدام Popcat AI Art API"""
    try:
...
def check_breach(email):
    """فحص تسريب البريد الإلكتروني عبر HaveIBeenPwned"""
    try:
        if not HIBP_API_KEY:
...
def fetch_temp_numbers_advanced(country_code='US', limit=5):
    """جلب أرقام هواتف مؤقتة حقيقية مع تفاصيل الدولة من مصادر متعددة"""
    results = []
    sources = [
...
def get_verification_code(phone_number, source_url):
    """محاولة جلب كود التفعيل للرقم من المصدر"""
    try:
